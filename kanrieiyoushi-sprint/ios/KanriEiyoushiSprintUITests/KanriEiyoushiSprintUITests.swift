...
final class KanriEiyoushiSprintUITests: XCTestCase {
    private func launch(premium: Bool = true) -> XCUIApplication {
        let app = XCUIApplication()
        app.launchArguments = premium ? ["-UITestPremium"] : []
        app.launch()
        return app
    }
    ...
    func testFourTabsAndDailySprintImmediateScoring() {
        ...
        XCTAssertTrue(app.otherElements["quizView"].waitForExistence(timeout: 10))
        XCTAssertTrue(app.staticTexts["questionPrompt"].exists)
    ...
    func testSettingsExposeGoldenMasterControls() {
        ...
        XCTAssertTrue(app.otherElements["settingsView"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["goal4Button"].exists)
    ...
    func testPremiumMockAndHistoryRoutes() {
        ...
        XCTAssertTrue(app.otherElements["mockView"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["mockRound1Button"].exists)
        ...
        XCTAssertTrue(app.otherElements["historyView"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["学習記録"].exists)
    }
}
