#!/bin/bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)";ROOT="$(cd "$SCRIPT_DIR/.." && pwd)";ASSET_DIR="$SCRIPT_DIR/Assets.xcassets/AppIcon.appiconset";ICON_SRC="$SCRIPT_DIR/AppIcon.png";ICON_DRIVE_ID="11d72Dl76UH7QvU8Gxl-SgDjTV73GaxP4";ICON_EXPECTED_SIZE="726223";ICON_SHA256="294481351106502f20958359d02bb2fb117ae18399654388425aad0e264fe31f";ICON_MODE="${KANRI_ICON_MODE:-canonical}"
python3 "$ROOT/scripts/build_native_questions.py";test -f "$SCRIPT_DIR/Resources/questions.native.json"
rm -rf "$SCRIPT_DIR/Assets.xcassets";mkdir -p "$ASSET_DIR"
case "$ICON_MODE" in
  skip) echo "INFO: AppIcon skipped for static audit.";;
...
PY
  ...
  canonical)
    ...
    cp "$ICON_SRC" "$ASSET_DIR/AppIcon.png";echo "PASS: canonical AppIcon SHA256=${ICON_SHA256}";;
