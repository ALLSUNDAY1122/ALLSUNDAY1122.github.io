...
for f in index.html styles.css styles-v061.css app-v061.js data-bootstrap-v061.js manifest.json questions.js questions-121-150.js questions-151-180.js questions-181-210.js questions-211-240.js; do
  test -f "$ROOT/$f"
  cp "$ROOT/$f" "$WEB_DST/$f"
done
cp "$ROOT"/data/*.js "$WEB_DST/data/"
for f in round2-extra-121-140.json round2-extra-141-160.json round2-extra-161-170.json round2-extra-171-180.json round2-extra-181-190.json round2-extra-191-200.json round2-rebalance-201-216.json round2-rebalance-217-232.json primary-source-registry.json; do
  cp "$ROOT/audit/$f" "$WEB_DST/audit/$f"
done
...
needle='<script src="data-bootstrap-v061.js?v=061"></script>'
...
'''
js=js[:pos]+api+js[pos:]
app.write_text(js,encoding='utf-8')
PY

case "$ICON_MODE" in
  skip)
    echo "INFO: AppIcon skipped for static/native UI audit."
    ;;
...
PY
  ...
  canonical)
    if [ ! -f "$ICON_SRC" ]; then
      curl --fail --location --silent --show-error "https://drive.usercontent.google.com/download?id=${ICON_DRIVE_ID}&export=download&confirm=t" --output "$ICON_SRC"
    fi
    actual_size="$(wc -c < "$ICON_SRC" | tr -d ' ')"
    if [ "$actual_size" != "$ICON_EXPECTED_SIZE" ]; then
      echo "ERROR: canonical AppIcon byte-size mismatch: $actual_size (expected $ICON_EXPECTED_SIZE)" >&2
      rm -f "$ICON_SRC"
      exit 1
    fi
    python3 - "$ICON_SRC" <<'PY'
...
assert b[:8]==b'\x89PNG\r\n\x1a\n','not PNG'
w,h=struct.unpack('>II',b[16:24]);assert (w,h)==(1024,1024),(w,h)
print('PASS: canonical PNG dimensions 1024x1024')
PY
    cp "$ICON_SRC" "$ASSET_DIR/AppIcon.png"
    echo "PASS: canonical Drive AppIcon materialized: ${ICON_DRIVE_ID}"
    ;;
...
import json,sys
r=Path(sys.argv[1])
assert 'native-storekit.js' in (r/'index.html').read_text(encoding='utf-8')
assert '__KANRI_NATIVE_API' in (r/'app-v061.js').read_text(encoding='utf-8')
round3=sum(len(json.loads(p.read_text(encoding='utf-8'))) for p in (r/'audit/round3').glob('*.json'))
assert round3==200,round3
assert len(list((r/'audit').glob('round2-extra-*.json')))==6
print('PASS: iOS local bundle contains StoreKit bridge and 600-question source set')
PY
