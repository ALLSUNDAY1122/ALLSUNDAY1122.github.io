...
def main():
    ...
    for q in data:
        round_no = int(q["round"])
        subject = q["subject"]
        if round_no not in (1,2,3) or subject not in EXPECTED:
            raise SystemExit(f"invalid round/subject: {q.get('id')}")
        round_pos[round_no] += 1
        per_round[round_no][subject] += 1
        ...
        native.append({
            ...
            "blanks": [],
            "declarationFields": [],
            "sourceText": q.get("short_reason") or None,
            "memoryPoint": q.get("memory_line", ""),
            "explanation": explanation,
            "sourceTitle": "一次資料" if source_url else None,
            "sourceURL": source_url,
            "sourceRefs": [source_url] if source_url else [],
            "sourceCheckedAt": AUDIT_DATE,
            "lawBaselineDate": reference_date,
            "contentVersion": CONTENT_VERSION,
            "premium": not is_free,
            "examRound": f"第{round_no}回",
            "questionNumber": str(round_pos[round_no]),
            "rightsBasis": q.get("rights_basis") or q.get("origin_type") or "独自作問・内部権利監査済み"
        })

    for r in (1,2,3):
        if dict(per_round[r]) != EXPECTED:
            raise SystemExit(f"round {r} distribution mismatch: {dict(per_round[r])}")
    if sum(1 for q in native if not q["premium"]) != 60:
        raise SystemExit("free count mismatch")
    if dict(free_seen) != {s:6 for s in SUBJECTS}:
        raise SystemExit(f"free subject split mismatch: {dict(free_seen)}")

    payload = {
        "schemaVersion": 1,
        "qualification": "管理栄養士国家試験",
        "bundleID": "jp.allsunday1122.kanrieiyoushi",
        "contentVersion": CONTENT_VERSION,
        "sourceAuditDate": AUDIT_DATE,
        "questions": native
    }
