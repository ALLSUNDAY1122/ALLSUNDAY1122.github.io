...
final class ScanLabGeotagPolicyTests: XCTestCase {
    ...
    func testPrivateAlwaysStripsLocation() { XCTAssertNil(ScanLabGeotagPolicy.normalized(visibility: .private, location: tokyo, label: "Tokyo")) }
    func testUnlistedAlwaysStripsLocation() { XCTAssertNil(ScanLabGeotagPolicy.normalized(visibility: .unlisted, location: tokyo, label: "Tokyo")) }
    func testPublicWithoutGeotagKeepsLocationNil() { XCTAssertNil(ScanLabGeotagPolicy.normalized(visibility: .public, location: nil, label: nil)) }
    ...
    func testPublicLocationTrimsAndBoundsLabel() {
        let normalized = ScanLabGeotagPolicy.normalized(visibility: .public, location: tokyo, label: "  " + String(repeating: "a", count: 100) + "  ")
        XCTAssertEqual(normalized?.latitude, tokyo.latitude); XCTAssertEqual(normalized?.longitude, tokyo.longitude); XCTAssertEqual(normalized?.label?.count, 80)
    }
