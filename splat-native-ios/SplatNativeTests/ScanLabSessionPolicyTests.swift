...
final class ScanLabSessionPolicyTests: XCTestCase {
    ...
    func testTransientRefreshFailureKeepsAuthenticatedContextWhenCachedSessionSurvives() {
        XCTAssertEqual(ScanLabSessionPolicy.recoveryDecision(hasCachedSessionAfterFailure: true), .keepAuthenticatedAndRetry)
    }
    func testMissingCachedSessionAfterRefreshFailureRequiresSignIn() {
        XCTAssertEqual(ScanLabSessionPolicy.recoveryDecision(hasCachedSessionAfterFailure: false), .requireSignIn)
    }
}
