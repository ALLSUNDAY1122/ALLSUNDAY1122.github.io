...
final class MeshDepthPayloadTests: XCTestCase {
    func testStripsSourceRowPaddingWithoutChangingFloatBytes() throws {
        ...
        let expected: [UInt8] = [0x00,0x00,0x80,0x3F, 0x00,0x00,0x00,0x40, 0x00,0x00,0x40,0x40, 0x00,0x00,0x80,0x40]
        ...
        let packed = source.withUnsafeBytes { bytes in
            MeshDepthPayload.tightlyPackedFloat32(baseAddress: bytes.baseAddress!, width: 2, height: 2, sourceRowBytes: stride)
        }
    ...
    func testRejectsImplausiblyLargeDepthFrameBeforeAllocating() {
        var sentinel: UInt8 = 0
        withUnsafePointer(to: &sentinel) { pointer in
            let base = UnsafeRawPointer(pointer)
            XCTAssertNil(MeshDepthPayload.tightlyPackedFloat32(
                baseAddress: base,
                width: 8_192,
                height: 8_192,
                sourceRowBytes: 8_192 * MemoryLayout<Float>.size
            ))
        }
    }
}