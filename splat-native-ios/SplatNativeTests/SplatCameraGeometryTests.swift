...
final class SplatCameraGeometryTests: XCTestCase {
    ...
    func testRobustFramingKeepsLargeSceneAtEstablishedRatioInsideCameraEnvelope() {
        ...
        XCTAssertEqual(framing.distance, 28, accuracy: 0.0001)
        XCTAssertEqual(framing.radius, 10, accuracy: 0.0001)
    }

    func testRobustFramingStillCapsPathologicalLargeSceneAtCameraEnvelope() {
        let points = stride(from: -30, through: 30, by: 3).map { x in
            SplatPoint(
                position: SIMD3<Float>(Float(x), 0, 0),
                color: .sRGBUInt8(SIMD3<UInt8>(128, 128, 128)),
                opacity: .linearFloat(1),
        ...
        XCTAssertLessThanOrEqual(framing.distance, 60)
    }
