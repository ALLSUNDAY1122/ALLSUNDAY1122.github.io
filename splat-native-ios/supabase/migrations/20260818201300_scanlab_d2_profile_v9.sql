...
language sql stable security definer set search_path = '' as $$
  select p.id,p.handle,p.display_name,p.bio,p.avatar_url,count(s.id)::bigint
  from public.scanlab_profiles p
  left join public.scanlab_scans s on s.owner_id=p.id and s.visibility='public' and s.status='published' and s.moderation_status='approved'
...
comment on function public.scanlab_public_profile(text) is 'D2-003 public profile projection. Intentionally excludes email, avatar_path and non-public scans.';
