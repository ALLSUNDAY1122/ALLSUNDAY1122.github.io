...
async function discoverOwnedObjects(userID: string) {
  ...
  while (pending.length) {
    ...
    let offset = 0;
    while (true) {
      const { data, error } = await admin.storage.from("scanlab-assets").list(prefix, { limit: 100, offset, sortBy: { column: "name", order: "asc" } });
...
Deno.serve(async (req) => {
  ...
  const { data: scans, error: scansError } = await admin.from("scanlab_scans").select("asset_path,preview_path").eq("owner_id", user.id);
  if (scansError) return json({ error: "delete_prepare_failed", retryable: true }, 503);

  const referenced = (scans ?? []).flatMap((scan) => [scan.asset_path, scan.preview_path]).filter((path): path is string => ownedPath(user.id, path));
