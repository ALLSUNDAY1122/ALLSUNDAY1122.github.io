...
export function parseScanLabFeedCursor(rawCursor){
  ...
  if(separatorIndex<=0||separatorIndex===rawCursor.length-1)return{value:null,error:"invalid_cursor"};
  const publishedAtRaw=rawCursor.slice(0,separatorIndex),id=rawCursor.slice(separatorIndex+1).toLowerCase(),millis=Date.parse(publishedAtRaw);
  if(!Number.isFinite(millis)||!UUID_PATTERN.test(id))return{value:null,error:"invalid_cursor"};
  return{value:{publishedAt:new Date(millis).toISOString(),id}};
}
export function makeScanLabFeedCursor(scan){
  const millis=Date.parse(scan?.published_at??""),id=String(scan?.id??"").toLowerCase();
