...
s=Path('SplatNative/ScanModel.swift').read_text(); resume=s.index('func resumeCapture()'); finish=s.index('func finishCapture()'); block=s[resume:finish]
...
# D2 durable browser asset URL: public metadata exposes a stable Edge Function URL,
# while the function mints short-lived Storage URLs only server-side and preserves Range requests.
require_file supabase/functions/scanlab-public/asset_delivery.mjs
require_file scripts/test_scanlab_public_asset_delivery.mjs
require_text supabase/functions/scanlab-public/index.ts 'mode === "asset"'
require_text supabase/functions/scanlab-public/index.ts 'req.headers.get("range")'
require_text supabase/functions/scanlab-public/index.ts 'parseScanLabAssetPath'
node scripts/test_scanlab_public_asset_delivery.mjs

plutil -lint SplatNative/PrivacyInfo.xcprivacy >/dev/null
python3 - <<'PY'
import plistlib
with open('SplatNative/PrivacyInfo.xcprivacy','rb') as f: data=plistlib.load(f)
assert data.get('NSPrivacyTracking') is False
assert data.get('NSPrivacyTrackingDomains') == []
by_type={item.get('NSPrivacyCollectedDataType'): item for item in data.get('NSPrivacyCollectedDataTypes', [])}
expected={'NSPrivacyCollectedDataTypeName','NSPrivacyCollectedDataTypeEmailAddress','NSPrivacyCollectedDataTypeUserID','NSPrivacyCollectedDataTypePreciseLocation','NSPrivacyCollectedDataTypeOtherUserContent'}
assert expected.issubset(by_type)
for kind in expected:
    item=by_type[kind]; assert item.get('NSPrivacyCollectedDataTypeLinked') is True; assert item.get('NSPrivacyCollectedDataTypeTracking') is False; assert 'NSPrivacyCollectedDataTypePurposeAppFunctionality' in item.get('NSPrivacyCollectedDataTypePurposes', [])
