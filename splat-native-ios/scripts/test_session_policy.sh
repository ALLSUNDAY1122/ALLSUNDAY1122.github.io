...
if grep -q '@testable import SplatNative' "$root/SplatNativeTests/ScanLabSessionPolicyTests.swift"; then
  echo 'session-policy regression failed: tests compile policy source directly and must not import SplatNative' >&2; exit 1
fi
cp "$root/SplatNative/ScanLabSessionPolicy.swift" "$tmp/ScanLabSessionPolicy.swift"
cat > "$tmp/main.swift" <<'SWIFT'
import Foundation
func expect(_ condition: @autoclosure () -> Bool, _ message: String) { guard condition() else { fputs("session-policy regression failed: \(message)\n", stderr); exit(1) } }
