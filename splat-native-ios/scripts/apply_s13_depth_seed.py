...
5. surface the actual seed source/counts in the training/result UI so physical evidence can distinguish depth from fallback.
...
RESULT_VIEW_PATH = ROOT / "SplatNative" / "SplatResultView.swift"
text = MODEL_PATH.read_text()
library = LIBRARY_PATH.read_text()
root_view = ROOT_VIEW_PATH.read_text()
result_view = RESULT_VIEW_PATH.read_text()

old_seed_property = """    @Published var depthCaptureActive = false\n    @Published private(set) var isWorldMapPersistencePending = false\n"""
new_seed_property = """    @Published var depthCaptureActive = false\n    @Published private(set) var reconstructionSeedEvidenceText: String?\n    @Published private(set) var isWorldMapPersistencePending = false\n"""
if old_seed_property in text:
    text = text.replace(old_seed_property, new_seed_property, 1)
elif new_seed_property not in text:
    raise SystemExit("S13: seed evidence property callsite drifted")
...
reset_marker = """        depthCaptureActive = false\n        systemPausedCapture = false\n"""
reset_with_seed = """        depthCaptureActive = false\n        reconstructionSeedEvidenceText = nil\n        systemPausedCapture = false\n"""
if reset_marker in text:
    text = text.replace(reset_marker, reset_with_seed, 1)
elif reset_with_seed not in text:
    raise SystemExit("S13: new-capture seed reset callsite drifted")
...
old_training_stage = """            Text(model.trainingStageText)\n                .font(.body.weight(.medium))\n            Text(\"\\(Int(model.trainingProgress * 100))%\")\n"""
new_training_stage = """            Text(model.trainingStageText)\n                .font(.body.weight(.medium))\n            if let seedEvidence = model.reconstructionSeedEvidenceText {\n                Text(seedEvidence)\n                    .font(.caption.monospacedDigit().weight(.semibold))\n                    .foregroundStyle(.mint)\n                    .multilineTextAlignment(.center)\n            }\n            Text(\"\\(Int(model.trainingProgress * 100))%\")\n"""
if old_training_stage in root_view:
    root_view = root_view.replace(old_training_stage, new_training_stage, 1)
elif new_training_stage not in root_view:
    raise SystemExit("S13: training evidence UI callsite drifted")

old_result_count = """                if viewerState.totalPointCount > 0 {\n                    Text(\"表示 \\(viewerState.visiblePointCount.formatted()) / \\(viewerState.totalPointCount.formatted()) splats\")\n                        .font(.caption.monospacedDigit())\n                        .foregroundStyle(.secondary)\n                }\n"""
new_result_count = """                if viewerState.totalPointCount > 0 {\n                    Text(\"表示 \\(viewerState.visiblePointCount.formatted()) / \\(viewerState.totalPointCount.formatted()) splats\")\n                        .font(.caption.monospacedDigit())\n                        .foregroundStyle(.secondary)\n                }\n                if let seedEvidence = model.reconstructionSeedEvidenceText {\n                    Text(seedEvidence)\n                        .font(.caption2.monospacedDigit().weight(.semibold))\n                        .foregroundStyle(.mint)\n                }\n"""
if old_result_count in result_view:
    result_view = result_view.replace(old_result_count, new_result_count, 1)
elif new_result_count not in result_view:
    raise SystemExit("S13: result evidence UI callsite drifted")

MODEL_PATH.write_text(text)
LIBRARY_PATH.write_text(library)
ROOT_VIEW_PATH.write_text(root_view)
RESULT_VIEW_PATH.write_text(result_view)
print("S13 geometry + same-raw A/B + seed telemetry materialization complete")
