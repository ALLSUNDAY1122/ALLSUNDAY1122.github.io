...
struct ScanLabProfile: Decodable, Hashable { let id: UUID; let handle: String; let displayName: String; let avatarPath: String?; enum CodingKeys: String, CodingKey { case id, handle; case displayName = "display_name"; case avatarPath = "avatar_path" } }
...
@MainActor final class ScanLabBackend: ObservableObject {
    ...
    @Published private(set) var isAuthenticated = false; @Published private(set) var currentUserEmail: String?; @Published private(set) var publicScans: [ScanLabPublicScan] = []; @Published private(set) var ownerScans: [ScanLabOwnerScan] = []; @Published private(set) var profile: ScanLabProfile?; @Published private(set) var isLoadingPublic = false; @Published private(set) var isLoadingOwner = false; @Published var notice: String?
    ...
    private func observeAuth() async { for await state in client.auth.authStateChanges { guard [.initialSession, .signedIn, .signedOut, .userUpdated].contains(state.event) else { continue }; isAuthenticated = state.session != nil; currentUserEmail = state.session?.user.email; if state.session == nil { ownerScans = []; profile = nil } else { async let scans: Void = loadOwnerScans(); async let loadedProfile: Void = loadProfile(); _ = await (scans, loadedProfile); await loadPublicScans() } } }
    ...
    func loadPublicScans(boundingBox: (minLat: Double, maxLat: Double, minLon: Double, maxLon: Double)? = nil) async { guard !isLoadingPublic else { return }; isLoadingPublic = true; defer { isLoadingPublic = false }; do { var components = URLComponents(url: ScanLabConfig.publicFunctionURL, resolvingAgainstBaseURL: false)!; var items = [URLQueryItem(name: "mode", value: "feed"), URLQueryItem(name: "limit", value: "40")]; if let boundingBox { items += [URLQueryItem(name: "minLat", value: String(boundingBox.minLat)), URLQueryItem(name: "maxLat", value: String(boundingBox.maxLat)), URLQueryItem(name: "minLon", value: String(boundingBox.minLon)), URLQueryItem(name: "maxLon", value: String(boundingBox.maxLon))] }; components.queryItems = items; guard let url = components.url else { throw ScanLabBackendError.invalidServerResponse }; var request = URLRequest(url: url); request.setValue("application/json", forHTTPHeaderField: "Accept"); if let session = try? await client.auth.session { request.setValue("Bearer \(session.accessToken)", forHTTPHeaderField: "Authorization") }; let (data, response) = try await URLSession.shared.data(for: request); guard let http = response as? HTTPURLResponse, 200..<300 ~= http.statusCode else { throw ScanLabBackendError.invalidServerResponse }; publicScans = try JSONDecoder().decode(ScanLabPublicEnvelope.self, from: data).items } catch { notice = "公開スキャンを取得できませんでした: \(error.localizedDescription)" } }
    func loadProfile() async { guard isAuthenticated, let session = try? await client.auth.session else { return }; do { let row: ScanLabProfile = try await client.from("scanlab_profiles").select("id,handle,display_name,avatar_path").eq("id", value: session.user.id).single().execute().value; profile = row } catch { notice = "プロフィールを取得できませんでした: \(error.localizedDescription)" } }
    ...
    func loadOwnerScans() async { guard isAuthenticated, !isLoadingOwner else { return }; isLoadingOwner = true; defer { isLoadingOwner = false }; do { let rows: [ScanLabOwnerScan] = try await client.from("scanlab_scans").select("id,title,caption,visibility,status,moderation_status,share_token,asset_path,preview_path,latitude,longitude,location_label,published_at,created_at").order("created_at", ascending: false).execute().value; ownerScans = rows } catch { notice = "自分のスキャンを取得できませんでした: \(error.localizedDescription)" } }
    ...
    func publish(resultURL: URL, previewImage: UIImage?, title: String, caption: String, visibility: ScanLabVisibility, location: ScanLabLocation?, publicPlaceConfirmed: Bool, privacyConfirmed: Bool, rightsConfirmed: Bool, contentConfirmed: Bool) async throws -> ScanLabPublishResponse {
        ...
        do { package = try ScanLabPublishPackageBuilder.build(from: resultURL, maximumBytes: ScanLabConfig.maximumAssetBytes) } catch ScanLabPublishPackageError.sourceTooLarge { throw ScanLabBackendError.assetTooLarge } catch { throw ScanLabBackendError.invalidAsset }
        defer { ScanLabPublishPackageBuilder.cleanup(package) }
        let uploadID = UUID().uuidString.lowercased(), prefix = "\(user.id.uuidString.lowercased())/\(uploadID)", assetPath = "\(prefix)/scene.spz", manifestPath = "\(prefix)/manifest.json", previewPath = previewImage == nil ? nil : "\(prefix)/preview.jpg"; var uploadedPaths: [String] = []
        do {
            ...
            if let previewPath, let previewData = previewImage?.jpegData(compressionQuality: 0.82) { try await client.storage.from("scanlab-assets").upload(previewPath, data: previewData, options: FileOptions(contentType: "image/jpeg")); uploadedPaths.append(previewPath) }
            ...
            let created: ScanLabCreatedScan = try await client.from("scanlab_scans").insert(draft).select("id").single().execute().value; let published: ScanLabPublishResponse = try await client.functions.invoke("scanlab-publish", options: FunctionInvokeOptions(region: .apSoutheast1, body: ScanLabPublishRequest(scanId: created.id.uuidString.lowercased()), timeoutInterval: 30)); await loadOwnerScans(); if visibility == .public { await loadPublicScans() }; return published
        } catch { if !uploadedPaths.isEmpty { try? await client.storage.from("scanlab-assets").remove(paths: uploadedPaths) }; throw error }
    }
    ...
    func delete(_ scan: ScanLabOwnerScan) async throws { let prefix = (scan.assetPath as NSString).deletingLastPathComponent; let paths = [scan.assetPath, "\(prefix)/manifest.json", scan.previewPath].compactMap { $0 }; if !paths.isEmpty { try await client.storage.from("scanlab-assets").remove(paths: paths) }; try await client.from("scanlab_scans").delete().eq("id", value: scan.id).execute(); await loadOwnerScans(); await loadPublicScans() }
