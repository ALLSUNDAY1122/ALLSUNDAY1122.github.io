...
struct ScanLabPublishManifest: Codable, Equatable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case sceneFile = "scene_file"
        case sceneByteCount = "scene_byte_count"
        case sceneSHA256 = "scene_sha256"
...
enum ScanLabPublishPackageBuilder {
    static func build(
    ...
    ) throws -> ScanLabPublishPackage {
        let attributes: [FileAttributeKey: Any]
        do {
            attributes = try fileManager.attributesOfItem(atPath: sourceURL.path)
        } catch {
            throw ScanLabPublishPackageError.invalidSource
        }
        guard let number = attributes[.size] as? NSNumber, number.int64Value > 0 else {
            throw ScanLabPublishPackageError.invalidSource
        }
        guard number.int64Value <= Int64(maximumBytes) else {
            throw ScanLabPublishPackageError.sourceTooLarge
        }

        let source: Data
        do {
            source = try Data(contentsOf: sourceURL, options: [.mappedIfSafe])
        } catch {
            throw ScanLabPublishPackageError.invalidSource
        }
        guard source.count == number.intValue, isDecodableSPZ(source) else {
            throw ScanLabPublishPackageError.invalidSPZ
        }

        let directory = fileManager.temporaryDirectory.appendingPathComponent(
            "scanlab-publish-\(UUID().uuidString.lowercased())",
            isDirectory: true
        )
        ...
        do {
            ...
            let manifest = ScanLabPublishManifest(
                schemaVersion: ScanLabPublishManifest.currentSchemaVersion,
            ...
            guard try verify(package, fileManager: fileManager) else {
    ...
    static func verify(
        ...
        guard isDecodableSPZ(scene),
              ...
              sha256(scene) == package.manifest.sceneSHA256 else {
            return false
        }

        let decoded = try JSONDecoder().decode(
            ScanLabPublishManifest.self,
    ...
    private static func isDecodableSPZ(_ data: Data) -> Bool {
        do {
            // SPZSceneReader.read() invokes spz-swift's loadSpzPacked before it
            // returns the stream, so malformed gzip/header/payload data is rejected here.
            _ = try SPZSceneReader(data).read()
            return true
        } catch {
            return false
        }
