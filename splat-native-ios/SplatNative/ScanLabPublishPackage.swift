...
enum ScanLabPublishPackageBuilder {
    static func build(from sourceURL: URL, maximumBytes: Int = 128 * 1024 * 1024, fileManager: FileManager = .default) throws -> ScanLabPublishPackage {
        ...
        guard source.count == number.intValue, looksLikeSPZ(source) else { throw ScanLabPublishPackageError.invalidSPZ }
        ...
        do {
            ...
            let manifest = ScanLabPublishManifest(
                ...
                sceneSHA256: SHA256.hash(data: source).map { String(format: "%02x", $0) }.joined(),
    ...
    static func verify(_ package: ScanLabPublishPackage, fileManager: FileManager = .default) throws -> Bool {
        guard package.manifest.schemaVersion == ScanLabPublishManifest.currentSchemaVersion,
              ...
              fileManager.fileExists(atPath: package.manifestURL.path) else { return false }
        let scene = try Data(contentsOf: package.sceneURL, options: [.mappedIfSafe])
        guard looksLikeSPZ(scene), Int64(scene.count) == package.manifest.sceneByteCount else { return false }
        let digest = SHA256.hash(data: scene).map { String(format: "%02x", $0) }.joined()
        guard digest == package.manifest.sceneSHA256 else { return false }
    ...
    private static func looksLikeSPZ(_ data: Data) -> Bool {
        // SPZ is gzip-wrapped. Reject arbitrary result.splat bytes before network upload.
        data.count >= 18 && data[data.startIndex] == 0x1f && data[data.startIndex + 1] == 0x8b && data[data.startIndex + 2] == 0x08
    }
}
