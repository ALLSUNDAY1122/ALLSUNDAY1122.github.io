...
enum BoundedFileReader {
    ...
    static func read(_ url: URL, maximumBytes: Int) throws -> Data {
        ...
        if let size = before.fileSize, Int64(size) != beforeIdentity.byteCount { throw BoundedFileReaderError.fileChangedDuringRead }
        ...
        var data = Data()
        let verifiedSize = Int(openedIdentity.byteCount)
        data.reserveCapacity(min(verifiedSize, maximumBytes, Self.reserveCapacityCeilingBytes))
        while data.count <= maximumBytes {
            ...
            guard chunk.count <= maximumBytes - data.count else { throw BoundedFileReaderError.fileTooLarge }
            data.append(chunk)
        }
        ...
        guard let afterIdentity = pathIdentity(at: url), let finalOpenedIdentity = handleIdentity(handle),
              beforeIdentity == afterIdentity, openedIdentity == finalOpenedIdentity,
              afterIdentity.linkCount == 1, finalOpenedIdentity.linkCount == 1,
              before.fileSize == after.fileSize, after.fileSize == data.count,
    ...
    private static func handleIdentity(_ handle: FileHandle) -> FileIdentity? {
        var info = stat()
        guard fstat(handle.fileDescriptor, &info) == 0, (info.st_mode & S_IFMT) == S_IFREG else { return nil }
        return identity(from: info)
    }

    private static func identity(from info: stat) -> FileIdentity {
        FileIdentity(systemNumber: UInt64(info.st_dev), fileNumber: UInt64(info.st_ino), linkCount: UInt64(info.st_nlink), byteCount: Int64(info.st_size), modificationSeconds: Int64(info.st_mtimespec.tv_sec), modificationNanoseconds: Int64(info.st_mtimespec.tv_nsec), changeSeconds: Int64(info.st_ctimespec.tv_sec), changeNanoseconds: Int64(info.st_ctimespec.tv_nsec))
    }
