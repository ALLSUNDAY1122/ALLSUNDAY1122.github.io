...
struct ScanReportDraft: Equatable, Sendable {
    ...
    init(scanID: UUID, reason: ScanReportReason, details: String) throws {
        guard details.count <= Self.maximumDetailsLength else { throw ScanReportValidationError.detailsTooLong }
        self.scanID = scanID; self.reason = reason; self.details = details
    }
}

enum ScanReportValidationError: Error, Equatable, Sendable { case detailsTooLong }
struct ScanReportReceipt: Decodable, Equatable, Sendable {
    let reportID: Int64
    let duplicate: Bool
    let createdAt: Date
    enum CodingKeys: String, CodingKey { case reportID = "report_id"; case duplicate; case createdAt = "created_at" }
}
...
enum ScanReportSubmissionState: Equatable, Sendable {
    ...
    var disablesSubmit: Bool {
        switch self { case .submitting, .persisted, .duplicate: return true; case .ready, .failed: return false }
    }
}

enum ScanReportPolicy {
    static func state(from receipt: ScanReportReceipt) -> ScanReportSubmissionState {
        receipt.duplicate ? .duplicate(reportID: receipt.reportID, createdAt: receipt.createdAt) : .persisted(reportID: receipt.reportID, createdAt: receipt.createdAt)
    }
}
