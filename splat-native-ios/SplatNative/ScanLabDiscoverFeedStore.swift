...
final class ScanLabDiscoverFeedStore: ObservableObject {
    ...
    func reload(using backend: ScanLabBackend) async {
        ...
        defer { if generation == requestGeneration { isLoadingInitial = false } }
        do {
            let page = try await fetchVisiblePage(startingAt: nil, query: requestQuery, using: backend)
            guard generation == requestGeneration else { return }
            items = unique(page.items); nextCursor = page.nextCursor; loadedQuery = requestQuery; hasMore = page.nextCursor != nil
        } catch {
            guard generation == requestGeneration else { return }
            if isCancellation(error) { return }
            items = []; nextCursor = nil; hasMore = false; errorMessage = error.localizedDescription
        }
    }
    ...
    func loadNextPage(using backend: ScanLabBackend) async {
        ...
        do {
            ...
            nextCursor = page.nextCursor; hasMore = page.nextCursor != nil; errorMessage = nil
        } catch {
            guard generation == requestGeneration else { return }
            if isCancellation(error) { return }
    ...
    private func fetchPage(cursor: String?, query: String, using backend: ScanLabBackend) async throws -> ScanLabDiscoverEnvelope {
        ...
        var queryItems = [URLQueryItem(name: "mode", value: "feed"), URLQueryItem(name: "limit", value: String(pageSize))]
        if let cursor, !cursor.isEmpty { queryItems.append(URLQueryItem(name: "cursor", value: cursor)) }
        if !query.isEmpty { queryItems.append(URLQueryItem(name: "q", value: String(query.prefix(80)))) }
        ...
        if let session = try? await backend.client.auth.session { request.setValue("Bearer \(session.accessToken)", forHTTPHeaderField: "Authorization") }
    ...
    private func normalizedQuery(_ raw: String) -> String { String(raw.trimmingCharacters(in: .whitespacesAndNewlines).prefix(80)) }
    private func isCancellation(_ error: Error) -> Bool { Task.isCancelled || (error as? URLError)?.code == .cancelled }
    private func unique(_ scans: [ScanLabPublicScan]) -> [ScanLabPublicScan] { var seen = Set<UUID>(); return scans.filter { seen.insert($0.id).inserted } }
}
