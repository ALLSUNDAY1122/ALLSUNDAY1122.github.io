...
struct MeshAdvancedSupervisor: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingTrim) {
            if let url = currentOBJ {
                MeshTrimEditorSheet(sourceURL: url).environmentObject(model)
            }
        }
        .sheet(isPresented: $showingAppearance) {
            if let url = currentOBJ {
                MeshAppearanceEditorSheet(sourceURL: url).environmentObject(model)
            }
        }
    ...
    private var scanningControls: some View {
        VStack(spacing: 0) {
            ...
            if model.canFinish && !captureQualitySatisfied {
                VStack(spacing: 7) {
                    Label("撮影品質がまだ不足", systemImage: "viewfinder")
                        .font(.subheadline.bold())
                    Text(qualityAdvisor.guidance)
                        .font(.caption)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    if model.currentMeshHasMetricScale {
                        actionButton("AR", systemImage: "arkit") { showingARViewer = true }
                    }
                    actionButton("トリミング", systemImage: "crop") { showingTrim = true }
                    if model.resultURL?.lastPathComponent.lowercased().contains("textured") == true ||
                        model.resultURL?.lastPathComponent.lowercased().contains("edited") == true {
                    ...
                    actionButton("軽量化", systemImage: "square.3.layers.3d.down.right") {
                        showingDetailSimplifier = true
                    }
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 14) {
                ...
                Text("写真だけから再構築したUSDZはARKit実寸座標へ校正していません。誤ったcm/m値を出さないため、この結果では計測と実寸AR配置を無効にしています。")
                    ...
                    .multilineTextAlignment(.center)
                HStack {
                    Button("Meshを書き出す") { showingUncalibratedShare = true }
