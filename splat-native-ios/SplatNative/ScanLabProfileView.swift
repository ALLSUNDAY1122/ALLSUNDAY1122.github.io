...
struct ScanLabProfileView: View {
            ...
            Section("公開プロフィール") {
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text(profile?.displayName ?? "プロフィール")
                            .font(.headline)
                ...
                Text("表示名・ユーザーID・自己紹介・アバターは公開プロフィールに表示されます。メールアドレスは公開されません。")
                    .font(.footnote).foregroundStyle(.secondary)
                Button("プロフィールを保存") { Task { await save() } }
                    .disabled(busy || !ScanLabProfilePolicy.validate(handle: handle, displayName: displayName, bio: bio, avatarURL: avatarURL))
    ...
    private func load() async {
        ...
        do {
            ...
            profile = loaded
            displayName = loaded.displayName; handle = loaded.handle; bio = loaded.bio ?? ""; avatarURL = loaded.avatarURL?.absoluteString ?? ""
    ...
    private func save() async {
        ...
        do {
            ...
            profile = saved
            displayName = saved.displayName; handle = saved.handle; bio = saved.bio ?? ""; avatarURL = saved.avatarURL?.absoluteString ?? ""
...
struct ScanLabPublicProfileView: View {
    ...
    private func load() async {
        ...
        do { profile = try await ScanLabProfileService(client: backend.client).loadPublic(handle: handle) }
        catch { errorMessage = error.localizedDescription }
    }
}
