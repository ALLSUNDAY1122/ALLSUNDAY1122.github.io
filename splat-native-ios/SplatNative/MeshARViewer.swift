    ...
    var body: some View {
        ZStack {
            ...
            if let url = model.resultURL {
                switch displayMode {
                case .ar:
                    MeshARPlacementView(modelURL: url, preparedScene: model.previewScene).ignoresSafeArea()
                case .object:
                    if let scene = model.previewScene {
                        SceneView(scene: scene, options: [.allowsCameraControl, .autoenablesDefaultLighting]).ignoresSafeArea()
                    }
                }
            }
            VStack(spacing: 0) {
                HStack {
                    Button { dismiss() } label: {
                        Image(systemName: "xmark").font(.headline).frame(width: 42, height: 42).background(.black.opacity(0.62), in: Circle())
                    }
                    Spacer()
                ...
                if displayMode == .ar {
                    Text("水平面をタップすると、実寸Meshをその位置へ置き直します")
                        ...
                        .background(.black.opacity(0.68), in: Capsule()).padding(.bottom, 18)
                }
...
struct MeshARPlacementView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject {
        ...
        private func place(at transform: simd_float4x4) {
            ...
            if let placedNode { placedNode.simdTransform = transform; return }
            let source: SCNScene
            ...
            let anchor = SCNNode(); anchor.simdTransform = transform
            let modelRoot = SCNNode(); modelRoot.addChildNode(source.rootNode.clone()); recenterForPlacement(modelRoot)
            anchor.addChildNode(modelRoot); view.scene.rootNode.addChildNode(anchor); placedNode = anchor
        }
        ...
        private func recenterForPlacement(_ root: SCNNode) {
            ...
            root.enumerateChildNodes { node, _ in
                ...
                for corner in Self.corners(minimum: bounds.min, maximum: bounds.max) {
                    let local = node.convertPosition(corner, to: root); let p = SIMD3<Float>(local.x, local.y, local.z)
                    guard p.x.isFinite, p.y.isFinite, p.z.isFinite else { continue }
                    minimum = simd_min(minimum, p); maximum = simd_max(maximum, p); found = true
                }
            }
        ...
        nonisolated static func translationOnlyPlacement(from raycastTransform: simd_float4x4) -> simd_float4x4 {
            var transform = matrix_identity_float4x4; let translation = raycastTransform.columns.3
            if translation.x.isFinite, translation.y.isFinite, translation.z.isFinite { transform.columns.3 = SIMD4<Float>(translation.x, translation.y, translation.z, 1) }
            return transform
        }
