(async()=>{
'use strict';
const parts=[1,2,3,4,5].map(n=>`./app-v0.7.2-${n}.part?v=074`);
try{
  ...
  source=source.replace("const VERSION='0.7.2';","const VERSION='0.7.4';");
  ...
  (0,eval)(source);
  document.title='Sprint Study v0.7.4';
  const badge=document.querySelector('.version-badge');
  if(badge)badge.textContent='v0.7.4';
}catch(error){
  console.error(error);
