...
WORKFLOW_BLOCK = r'''
  ...
  scanner-parity-golden-v3-qa:
    name: Scanner Parity - Formal Golden v3 QA
    max_build_duration: 120
    instance_type: mac_mini_m2
    environment:
      xcode: latest
    scripts:
      - name: Run Formal Golden v3 ephemeral QA
        script: |
          set -euo pipefail
          cd "$CM_BUILD_DIR"
          bash scanner-parity/HQGoldenRunner/run-formal-golden-v3-codemagic-ephemeral.sh
'''
...
def main() -> int:
    ...
    if command.get("mode") not in {"thresholdless", "thresholded"}:
        raise SystemExit("Invalid Golden mode")
    ...
    with tempfile.TemporaryDirectory(prefix="scanner-golden-prep-") as temp:
        work = Path(temp) / "repo"
        run("git", "worktree", "add", "--detach", str(work), f"origin/{INTEGRATION_BRANCH}")
        try:
            helper = work / "scanner-parity/HQGoldenRunner/run-formal-golden-v3-codemagic-ephemeral.sh"
            if not helper.is_file():
                raise SystemExit("Ephemeral Golden executor is not integrated")
            ...
            if MARKER not in text:
                ...
                run("git", "commit", "-m", "scanner-parity: prepare Formal Golden Codemagic QA workflow", cwd=work)
            ...
            if MARKER not in yaml_path.read_text(encoding="utf-8"):
                raise SystemExit("Codemagic Golden workflow preparation failed")
            Path("/tmp/scanner-golden-prepared-sha.txt").write_text(prepared + "\n", encoding="utf-8")
            print(f"Prepared scanner-parity Golden Codemagic workflow at {prepared}")
        finally:
