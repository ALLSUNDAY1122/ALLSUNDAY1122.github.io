...
# Stable Codemagic application backing this monorepo. The same ID is recorded in
# canonical repository evidence and is preferred over /apps list discovery,
# whose response stopped returning a discoverable repository match in Aug 2026.
...
def main() -> int:
    ...
    registered = False
    try:
        # Resolve the already-proven monorepo application before issuing a
        # one-time relay token so failures here cannot consume source access.
