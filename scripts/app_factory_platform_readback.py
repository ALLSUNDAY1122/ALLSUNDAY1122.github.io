...
def main():
    ...
    issuer=os.environ["ASC_ISSUER_ID"]; keyid=os.environ["ASC_KEY_ID"]
    kp,cleanup=load_private_key()
    try:
        token=make_token(issuer,keyid,kp)
        for label,(app_id,bundle_id) in APPS.items():
            entry={"app_id":app_id,"bundle_id":bundle_id}
            for name,path in {
                "builds":f"/v1/apps/{app_id}/builds?sort=-uploadedDate&limit=20",
    ...
    if not cm_token:
        ...
        result["codemagic"]["apps_http_status"]=status
        result["codemagic"]["repository_candidates"]=candidates
        if candidates:
            ...
            result["codemagic"]["builds_http_status"]=bstatus
            result["codemagic"]["builds"]=sanitize(builds)

    OUT.write_text(json.dumps(sanitize(result),ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    print(json.dumps({"completed_at":result["completed_at"],"testflight_apps":list(result["testflight"]),"codemagic_apps_http":result["codemagic"].get("apps_http_status"),"codemagic_candidates":len(result["codemagic"].get("repository_candidates") or [])},ensure_ascii=False))
