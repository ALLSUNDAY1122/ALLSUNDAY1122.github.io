...
TARGET_VERSION = "1.0"
COMMAND_PATH = Path("automation/app2-004-yakuzaishi-final-submit-command.json")
...
def approved_target_build() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Unable to read approval command: {exc}") from exc
    if str(command.get("app_id")) != APP_ID or str(command.get("bundle_id")) != BUNDLE_ID:
        raise RuntimeError("Approval command target mismatch")
    if str(command.get("version")) != TARGET_VERSION:
        raise RuntimeError("Approval command version mismatch")
...
def main() -> None:
    target_build = approved_target_build()
    ...
    try:
        ...
        versions = [
            x for x in many(versions_payload)
            if (x.get("attributes") or {}).get("platform") == "IOS"
            and (x.get("attributes") or {}).get("versionString") == TARGET_VERSION
        ]
        if len(versions) != 1:
            raise RuntimeError("Target App Store version is not unique")
        version = versions[0]
        version_id = str(version["id"])
        if state(version) not in {"PREPARE_FOR_SUBMISSION", "READY_FOR_REVIEW", "WAITING_FOR_REVIEW", "IN_REVIEW"}:
            raise RuntimeError(f"Unexpected version state: {state(version)}")
        ...
        if str(attrs.get("version")) != target_build or attrs.get("processingState") != "VALID":
            raise RuntimeError(
                f"Selected build is not approved VALID Build {target_build}: "
                f"{attrs.get('version')} / {attrs.get('processingState')}"
            )
        if attrs.get("expired") is True:
            raise RuntimeError(f"Approved Build {target_build} is expired")
        ...
        complete = [
            ...
            and (((x.get("attributes") or {}).get("assetDeliveryState") or {}).get("state")) == "COMPLETE"
        ]
        if not complete:
            raise RuntimeError("No COMPLETE App Store screenshot")
        ...
        review = one(review_payload, "review detail")
        ra = review.get("attributes") or {}
        for key in ("contactFirstName", "contactLastName", "contactPhone", "contactEmail", "notes"):
            if not ra.get(key):
                raise RuntimeError(f"App Review detail missing {key}")
        if ra.get("demoAccountRequired") is True:
            raise RuntimeError("Demo account must not be required")
        ...
        if already:
            result.update({
                "submitted": True,
                "idempotent": True,
                "review_submission_id": str(already["id"]),
                "review_submission_state": state(already),
            })
            output.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
            return
        draft = next((x for x in submissions if state(x) == "READY_FOR_REVIEW"), None)
        if not draft:
            raise RuntimeError("READY_FOR_REVIEW submission draft missing")
        sid = str(draft["id"])
        _, items_payload = api_get(token, f"/v1/reviewSubmissions/{sid}/items?limit=200")
        has_version = any(
            ((((x.get("relationships") or {}).get("appStoreVersion") or {}).get("data") or {}).get("id") == version_id)
            for x in many(items_payload)
        )
        if not has_version:
            raise RuntimeError("Review Submission does not contain target App Store version")
        ...
        after = one(after_payload, "review submission after submit")
        after_state = state(after)
        if after_state not in SUBMITTED_STATES:
            raise RuntimeError(f"Unexpected state after submit: {after_state}")
        result.update({
            "submitted": True,
            "idempotent": False,
