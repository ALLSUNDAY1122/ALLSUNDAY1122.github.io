#!/usr/bin/env python3
"""Upload the FP3 lifetime-IAP App Store review screenshot.

Narrow/fail-closed by design:
- exact app, bundle, IAP and product identities are verified first;
- an existing COMPLETE review screenshot is left untouched;
- an existing incomplete/unknown screenshot causes failure rather than replacement;
- the uploaded asset must reach COMPLETE and be readable back from the IAP.

Set FP3_IAP_REVIEW_SCREENSHOT to the audited PNG path.
"""
...
OUT = Path("fp3-iap-review-screenshot-result.json")
...
def main() -> None:
    screenshot = Path(os.environ.get("FP3_IAP_REVIEW_SCREENSHOT", ""))
    result = {"app_id": APP_ID, "iap_id": IAP_ID, "ok": False}
    cleanup = None
    try:
        if not screenshot.is_file():
            raise RuntimeError("FP3_IAP_REVIEW_SCREENSHOT must point to an audited screenshot file")
        if screenshot.suffix.lower() != ".png":
            raise RuntimeError("FP3 IAP review screenshot must be PNG")
        if screenshot.stat().st_size <= 0:
            raise RuntimeError("FP3 IAP review screenshot is empty")
            ...
            payload = {
                "data": {
                    ...
                    "attributes": {
                        ...
                        "fileName": "fp3-premium-review.png",
                    },
                    "relationships": {
                        "inAppPurchaseV2": {
