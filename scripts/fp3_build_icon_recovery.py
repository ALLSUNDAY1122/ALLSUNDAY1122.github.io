...
variants are byte-identical. Distinct variants are preserved as evidence and fail
closed instead of choosing one arbitrarily.
"""
...
def render_url(template: str, width: int, height: int) -> str:
    url = template
    replacements = {"{w}": str(width), "{h}": str(height), "{f}": "png", "{c}": ""}
...
def write_manifest(*, build: dict, build_id: str, variants: list[dict], status: str, canonical: dict | None = None) -> None:
    result = {
        ...
        "all_app_store_icons_byte_identical": len({x["png_sha256"] for x in variants}) == 1,
        "variants": variants,
...
def main() -> None:
    ...
    try:
        ...
        candidates = [
            item for item in (builds_payload.get("data") or [])
            if str((item.get("attributes") or {}).get("version")) == TARGET_BUILD_NUMBER
        ]
        if len(candidates) != 1:
        ...
        icons = [
            item for item in (icons_payload.get("data") or [])
            if str((item.get("attributes") or {}).get("iconType") or "") == "APP_STORE"
        ]
        if not icons:
        ...
        for index, (metadata, png) in enumerate(recovered, start=1):
            prerendered = str(bool(metadata.get("icon_masked"))).lower()
            Path(f"fp3-build9-appstore-icon-variant-{index}-prerendered-{prerendered}.png").write_bytes(png)

        unique_hashes = {metadata["png_sha256"] for metadata in variants}
        if len(unique_hashes) != 1:
            write_manifest(build=build, build_id=build_id, variants=variants, status="DISTINCT_VARIANTS")
            raise RuntimeError(
                "Distinct APP_STORE BuildIcon variants preserved for visual audit; refusing arbitrary selection: "
                + json.dumps([
                    {
                        "build_icon_id": x["build_icon_id"],
                        "icon_masked": x["icon_masked"],
        ...
        OUT_PNG.write_bytes(recovered[0][1])
        write_manifest(build=build, build_id=build_id, variants=variants, status="CANONICAL_RECOVERED", canonical=canonical)
