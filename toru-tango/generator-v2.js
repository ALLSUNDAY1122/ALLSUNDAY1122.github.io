(() => {
  ...
  function extractFacts(sentence) {
    ...
    if (/西方/.test(sentence) && /東方/.test(sentence) && /分かれて/.test(sentence)) {
      facts.push(makeFact('ユーラシアへ広がった人類は、どの二方向に分かれたと考えられているか', '西方と東方', sentence, 93, 'migration-directions'));
    }
  ...
  function generateQuestionsV2(text, count = 10, type = 'mix', difficulty = 'normal') {
    ...
    const facts = [...extractStructuredFacts(repaired), ...splitSentencesV2(repaired).flatMap(extractFacts)]
