(() => {
  ...
  const makeFact = (question, answer, source, priority = 50, factKey = '') => {
    const normalizedQuestion = normalizeText(question).replace(/[。]+$/, '');
    return {
  ...
  function extractStructuredFacts(text) {
    ...
    if (/継続入院所得保障保険/.test(compact) && /14日以上継続入院/.test(compact)) {
      add('継続入院所得保障保険の主な支払事由は何か', '14日以上継続入院', 100, 'insurance-hospitalization-14');
    }
    if (/メンタル疾病.*14日以上[継楼]続入院/.test(compact)) {
      add('メンタル入院所得保障充実型で給付対象となる入院は何か', 'メンタル疾病で14日以上継続入院', 99, 'mental-hospitalization-14');
    }
    if (/給付金月額6か月分/.test(compact)) {
      add('継続入院所得保障保険では、1回の継続入院で給付金月額の何か月分を受け取れるか', '6か月分', 98, 'benefit-six-months');
    }
    if (/給付金月額2か月分/.test(compact)) {
      add('メンタル疾病による継続入院では、追加で給付金月額の何か月分を受け取れるか', '2か月分', 97, 'benefit-two-months');
    }
    ...
    if (/就業不能保険/.test(compact) && /給付金月額30万/.test(compact)) {
      add('参考の就業不能保険の給付金月額上限はいくらか', '30万円', 90, 'reference-disability-max');
    }
    ...
    if (/(?:保険|保障)見直し/.test(compact) && /この保険からの見直し/.test(compact) && /この保険への見直し/.test(compact)) {
      add('保障見直しの取扱範囲は何か', 'この保険からの見直しと、この保険への見直しの両方', 86, 'policy-review');
    }
    ...
    if (/給付金月額10万.{0,30}?就業不能保険.{0,10}?通算/.test(compact) || /給付金月額10万\(「?就業不能保険」?と通算\)/.test(compact)) {
      add('最高給付金月額は、どの保険と通算されるか', '就業不能保険', 92, 'combined-policy');
    }
    if (/被保険者を従業員/.test(compact) && /受取人を法人/.test(compact)) {
      add('法人契約では、被保険者と受取人をそれぞれ誰にするか', '被保険者は従業員、受取人は法人', 89, 'corporate-roles');
    }
  ...
  function extractLocation(sentence, eventWord) {
    const beforeEvent = eventWord && sentence.includes(eventWord)
      ? sentence.split(eventWord)[0]
      : sentence;
    const candidates = [...beforeEvent.matchAll(
      /(?:^|[、，])([^、，]{2,70}?(?:大陸|州|国|地方|地域|村付近|村|付近|半島|沿岸|タンザニア|エチオピア|アフリカ|ヨーロッパ|アジア))(?:で|では)[、，]?/g
    )];
    if (!candidates.length) return '';
    return cleanEntity(candidates.at(-1)[1])
  ...
  function extractFacts(sentence) {
    ...
    if (named) {
      ...
      if (age) {
        facts.push(makeFact(`「${name}」と名付けられた${objectName}は、約何年前のものか`, age[0], sentence, 100, `name-age-${name}`));
      }
      if (exactDate) {
        facts.push(makeFact(`「${name}」が発見されたのはいつか`, exactDate[0], sentence, 99, `name-date-${name}`));
      }
      const location = extractLocation(sentence, '発見');
      if (location) {
        facts.push(makeFact(`「${name}」が発見された場所はどこか`, location, sentence, 98, `name-location-${name}`));
      }
    }
    ...
    if (birth) {
      ...
      if (subject && birth[2]) {
        facts.push(makeFact(`${subject}が誕生したとされるのは約何年前か`, birth[2], sentence, 97, `birth-age-${normalizeKey(subject)}`));
      }
      if (subject && location) {
        facts.push(makeFact(`${subject}はどこで誕生したとされているか`, location, sentence, 96, `birth-location-${normalizeKey(subject)}`));
      }
    }
    ...
    for (const match of sentence.matchAll(eventPattern)) {
      ...
      if (subject) {
        facts.push(makeFact(`${subject}が${match[2]}したのは約何年前か`, match[3], sentence, 97, `event-${match[2]}-${normalizeKey(subject)}`));
      }
    }
    ...
    if (ageFirstEvent) {
      ...
      if (subject) {
        facts.push(makeFact(`${subject}が${ageFirstEvent[4]}したのは約何年前か`, ageFirstEvent[1], sentence, 95, `age-first-${ageFirstEvent[4]}-${normalizeKey(subject)}`));
      }
      const location = cleanEntity(ageFirstEvent[2].replace(/で[、，]?$/, ''));
      if (subject && location && /(?:大陸|州|国|地方|地域|村|半島|沿岸|アフリカ|ヨーロッパ|アジア)/.test(location)) {
        facts.push(makeFact(`${subject}が${ageFirstEvent[4]}した場所はどこか`, location, sentence, 90, `age-first-location-${normalizeKey(subject)}`));
      }
    }
    ...
    if (!facts.length) {
      ...
      if (definition) {
        ...
        if (subject && answer && !new RegExp(TIME_EXPRESSION).test(answer)) {
          facts.push(makeFact(`${subject}とは何か`, answer, sentence, 70, `definition-${normalizeKey(subject)}`));
        }
      }
    }
  ...
  function generateQuestionsV2(text, count = 10, type = 'mix', difficulty = 'normal') {
    ...
    const facts = [
      ...extractStructuredFacts(repaired),
      ...splitSentencesV2(repaired).flatMap(extractFacts)
  ...
  globalThis.ToruTangoGeneratorV2 = {
    generateQuestionsV2,
    repairOcrText,
    splitSentencesV2,
    extractFacts,
