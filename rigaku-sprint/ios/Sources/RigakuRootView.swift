...
private struct RigakuHomeView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        .buttonStyle(.plain)
                        .disabled(!appModel.canStudy || appModel.weakCount == 0)
                        .accessibilityIdentifier("home.weak")

                        if !appModel.canStudy {
                            contentGate
                        }
        ...
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("今日の学習")
                    ...
                    .foregroundStyle(LearningSprintTheme.ink2)
                Text(appModel.todayAnsweredCount >= appModel.state.dailyTarget ? "今日の目標を達成しました" : "あと\(max(0, appModel.state.dailyTarget - appModel.todayAnsweredCount))問で今日の1セット完了")
    ...
    private var contentGate: some View {
        VStack(alignment: .leading, spacing: 6) {
            Label("問題品質ゲート中", systemImage: "checkmark.shield")
                .font(LearningSprintTheme.sans(13, weight: .bold))
                .foregroundStyle(LearningSprintTheme.indigo)
            Text("第60・59・58回の総枠600問は確認済みです。科目分類・権利・正答・独自解説を監査した問題だけ有効化します。")
    ...
    private var summarySection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("これまで")
                .font(LearningSprintTheme.sans(16, weight: .bold))
            HStack(spacing: 10) {
                RigakuSummaryCell(label: "解答", value: "\(appModel.state.attempts.count)")
                RigakuSummaryCell(label: "正答率", value: accuracyLabel)
                RigakuSummaryCell(label: "苦手", value: "\(appModel.weakCount)")
            }
        }
    }
...
private struct RigakuMockListView: View {
    @EnvironmentObject private var appModel: RigakuAppModel
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        Text("第60・59・58回は各200問の公式出題枠を確認済み。収録問題は個別権利監査後に有効化します。")
                            .font(LearningSprintTheme.sans(13))
                            .foregroundStyle(LearningSprintTheme.ink2)

                        ForEach(RigakuAppConfiguration.examRounds) { exam in
                            NavigationLink {
                                RigakuStudyView(kind: .mock(String(exam.round)))
                            } label: {
                                HStack {
                                    VStack(alignment: .leading, spacing: 4) {
                                        Text("第\(exam.round)回")
                                            .font(LearningSprintTheme.serif(20, weight: .bold))
                                        Text(exam.officialQuestionCount.map { "公式PDF確認済み：\($0)問" } ?? "問題数要確認")
                                            .font(LearningSprintTheme.sans(12, weight: .medium))
                                            .foregroundStyle(LearningSprintTheme.ink2)
                                    }
                                    Spacer()
                                    Image(systemName: exam.publicationStatus == .verifiedPublished ? "checkmark.seal" : "lock")
                                        .foregroundStyle(exam.publicationStatus == .verifiedPublished ? LearningSprintTheme.green : LearningSprintTheme.gold)
                                }
                                .padding(16)
                                .background(LearningSprintTheme.card)
                                .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
...
private struct RigakuHistoryView: View {
    @EnvironmentObject private var appModel: RigakuAppModel
...
private struct RigakuSettingsView: View {
    @EnvironmentObject private var appModel: RigakuAppModel
            ...
            ZStack {
                ...
                Form {
                    ...
                    Section("アプリ") {
                        ...
                        LabeledContent("収録済み", value: "\(appModel.questions.count)問")
                        LabeledContent("Bundle ID", value: RigakuAppConfiguration.runtimeBundleIdentifier ?? "要確認")
                        Text("App Store Connect App ID / IAP Product ID は正本値が確認できるまで実装へ埋め込みません。")
                            .font(.caption)
                            .foregroundStyle(LearningSprintTheme.ink3)
                    }
                }
...
private struct RigakuSummaryCell: View {
    let label: String
    let value: String
