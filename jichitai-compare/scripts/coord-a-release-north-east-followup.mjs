...
for (const [region, team] of regions) {
  ...
  for (const taskPath of listing ? listing.split('\n') : []) {
    ...
    let task;
    try { task = showJson(ref, taskPath); } catch (error) {
      excluded.push({ region, taskPath, reason: `task parse: ${error.message}` }); continue;
    }
    ...
    let municipality;
    try { municipality = showJson(ref, municipalityPath); } catch (error) {
      excluded.push({ region, code, name: task.municipalityName, reason: `municipality missing: ${error.message}` }); continue;
    }
    ...
    const normalizedTask = {
      ...
      notes: [...(Array.isArray(task.notes) ? task.notes : []), '統括Aの北日本・東日本追補監査によりmainへ全国統合。']
    };
...
writeFileSync(join(project, 'operations', 'control', 'coord-a-release-north-east-followup-20260724.json'), `${JSON.stringify({
  schemaVersion: '1.0.0', createdAt: releaseAt,
  previousPublishedMunicipalities: before.registeredMunicipalities,
  selectedMunicipalities: selected.length, byRegion, municipalities: selected,
  excludedCount: excluded.length, excluded
}, null, 2)}\n`);
