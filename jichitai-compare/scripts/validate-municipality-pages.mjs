...
const VALIDATION_REPORT_FILE = join(PROJECT_DIR, '.artifacts', 'public-copy-validation-report.json');
...
if (errors.length > 0 || auditViolations.length > 0) {
  await mkdir(dirname(VALIDATION_REPORT_FILE), { recursive: true });
  await writeFile(
...
if (auditViolations.length > 0) {
  console.warn(`公開文面の監査対象語を${auditViolations.length}件検出しました。レポートを確認してください。`);
}
...
if (errors.length > 0) {
  ...
  console.error(`検証レポート: ${VALIDATION_REPORT_FILE}`);
  process.exit(1);
}
