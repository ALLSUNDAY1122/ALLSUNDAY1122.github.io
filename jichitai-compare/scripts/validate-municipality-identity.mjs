...
async function readJson(path) {
  try {
    ...
    error(`${relative(PROJECT_DIR, path)}: JSONを読み込めません: ${cause.message}`);
    return null;
  }
}
...
async function main() {
  ...
  for (const filePath of files) {
    ...
    if (!/^\d{5}$/u.test(code)) {
      error(`${path}: codeは5桁の数字文字列にしてください`);
      continue;
    }
    if (!/^\d{2}$/u.test(prefectureCode)) error(`${path}: prefectureCodeは2桁の数字文字列にしてください`);
    if (code.slice(0, 2) !== prefectureCode) {
      error(`${path}: 自治体コード先頭2桁とprefectureCodeが一致しません: code=${code}, prefectureCode=${prefectureCode}`);
    }
    if (basename(filePath, '.json') !== code) error(`${path}: ファイル名と自治体コードが一致しません`);
    if (basename(dirname(filePath)) !== prefectureCode) error(`${path}: 親ディレクトリとprefectureCodeが一致しません`);

    const expectedPrefecture = PREFECTURE_NAMES.get(prefectureCode);
    if (!expectedPrefecture) error(`${path}: 未知の都道府県コードです: ${prefectureCode}`);
    else if (prefecture !== expectedPrefecture) {
      error(`${path}: 都道府県名がコードと一致しません: 期待=${expectedPrefecture}, 実際=${prefecture}`);
    }
