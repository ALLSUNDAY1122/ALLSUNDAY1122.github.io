...
async function main() {
  ...
  for (const filePath of files) {
    ...
    if (code.slice(0, 2) !== prefectureCode) addIssue('code_prefecture_mismatch', `${path}: 自治体コード先頭2桁とprefectureCodeが一致しません`, { code, prefectureCode });
    ...
    else if (prefecture !== expectedPrefecture) addIssue('prefecture_name_mismatch', `${path}: 都道府県名がコードと一致しません: 期待=${expectedPrefecture}, 実際=${prefecture}`, { record });
