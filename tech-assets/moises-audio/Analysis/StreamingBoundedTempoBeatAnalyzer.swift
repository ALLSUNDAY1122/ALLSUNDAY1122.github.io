...
public enum StreamingBoundedTempoBeatAnalyzer {
    ...
    public static func analyzeCancellable(
        ...
        try analyzeInternal(reader: reader, configuration: configuration, cancellationChecksEnabled: true)
    }
    ...
    private static func analyzeInternal(
        ...
        let onset = try onsetEnvelope(reader: reader, frameSize: frameSize, hopSize: hopSize, cancellationChecksEnabled: cancellationChecksEnabled)
        ...
        let envelopeRate = reader.sampleRate / Double(hopSize)
        ...
        for lag in minLag...maxLag {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: lag - minLag, stride: 1)
            let correlation = try normalizedAutocorrelation(onset, lag: lag, cancellationChecksEnabled: cancellationChecksEnabled)
            guard correlation > 0.025 else { continue }
            let phaseAndAlignment = try bestPhase(onset: onset, lag: lag, maxOnset: maxOnset, cancellationChecksEnabled: cancellationChecksEnabled)
        ...
        let hopSeconds = Double(hopSize) / reader.sampleRate
        let onsetOffsetSeconds = Double(frameSize) / reader.sampleRate
        let beatTimes = try trackedBeatTimes(onset: onset, lag: best.lag, phase: best.phase, hopSeconds: hopSeconds, onsetOffsetSeconds: onsetOffsetSeconds, durationSeconds: reader.durationSeconds, cancellationChecksEnabled: cancellationChecksEnabled)
    ...
    private static func onsetEnvelope(reader: AnalysisPreparedSampleReader, frameSize: Int, hopSize: Int, cancellationChecksEnabled: Bool) throws -> [Double] {
        ...
        while start + frameSize <= reader.sampleCount {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: frameIndex, stride: AnalysisCancellationPolicy.tempoFrameCheckStride)
            var sumSquares = 0.0
        ...
        if floorValue > 0 {
            for index in flux.indices {
                try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: index, stride: AnalysisCancellationPolicy.tempoFrameCheckStride * 4)
    ...
    private static func normalizedAutocorrelation(_ values: [Double], lag: Int, cancellationChecksEnabled: Bool) throws -> Double {
        ...
        while index < pairCount {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: iteration, stride: AnalysisCancellationPolicy.tempoCorrelationCheckStride)
    ...
    private static func bestPhase(onset: [Double], lag: Int, maxOnset: Double, cancellationChecksEnabled: Bool) throws -> (phase: Int, alignment: Double) {
        ...
        for (phaseIndex, phase) in phases.enumerated() {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: phaseIndex, stride: 1)
            var score = 0.0, count = 0, position = phase
            while position < onset.count {
                try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: count, stride: AnalysisCancellationPolicy.tempoPhaseCheckStride)
    ...
    private static func resolveMetricalCandidate(_ candidates: [Candidate]) -> Candidate? {
        ...
        for candidate in candidates.dropFirst().prefix(12) {
            ...
            if candidate.correlation >= selected.correlation * 0.88, candidate.gridAlignment >= selected.gridAlignment + 0.08, candidate.weightedScore >= selected.weightedScore * 0.94 { selected = candidate }
        }
        return selected
    }

    private static func trackedBeatTimes(onset: [Double], lag: Int, phase: Int, hopSeconds: Double, onsetOffsetSeconds: Double, durationSeconds: Double, cancellationChecksEnabled: Bool) throws -> [Double] {
        ...
        while predicted < onset.count {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: beatIndex, stride: AnalysisCancellationPolicy.tempoPhaseCheckStride)
        ...
        for (offset, index) in indices.enumerated() {
            try AnalysisCancellationPolicy.checkIfNeeded(enabled: cancellationChecksEnabled, iteration: offset, stride: AnalysisCancellationPolicy.tempoPhaseCheckStride)
