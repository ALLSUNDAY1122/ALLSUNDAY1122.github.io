...
public enum MusicalKeyAnalyzer {
    ...
    private static let modalDefinitions: [(
        name: String,
        scale: [Int],
        altered: Int,
        conventional: Int,
    ...
    public static func analyze(
        ...
        let availableWindows = max(
            1,
            (samples.count - windowSize) / max(1, configuration.analysisHopSize) + 1
        )
        let selectedCount = min(configuration.maximumKeyWindows, availableWindows)
        let starts = uniformlySpacedWindowStarts(
        ...
        if let relative = relativeCounterpart(of: global.best, in: ranked) {
            ...
            if relativeGap < configuration.keyRelativeAmbiguityMargin, tonicGap < 0.035 {
                return nil
            }
        }
        ...
        if global.activePitchClasses >= 5,
           ...
           modal.score >= 0.72 {
            // Current-iPhone modal vocabulary is not yet verified. Prefer unknown to
            // forcing a wrong major/minor label when the chroma is clearly modal.
            return nil
        }
        ...
        if let first = keyEvidence(halves.first),
           ...
           (first.best.tonic != second.best.tonic || first.best.mode != second.best.mode) {
            return nil
        } else if let first = keyEvidence(halves.first),
                  let second = keyEvidence(halves.second) {
            let agreeingHalves = [first.best, second.best].filter {
                $0.tonic == global.best.tonic && $0.mode == global.best.mode
            }.count
            temporalAgreement = Double(agreeingHalves) / 2.0
        }

        let calibratedMargin = min(1, global.normalizedMargin / 0.08)
        let confidence = clamp01(
            (calibratedMargin * 0.68)
                + (global.best.triadSupport * 0.20)
                + (temporalAgreement * 0.12)
        )
        guard confidence >= configuration.minimumKeyConfidence else { return nil }

        return MusicalKey(
            tonicPitchClass: global.best.tonic,
            mode: global.best.mode,
    ...
    private static func rankedCandidates(_ chroma: [Double]) -> [KeyCandidate] {
        ...
        for tonic in 0..<12 {
            for (mode, profile, third) in [
                ("major", majorProfile, 4),
                ("minor", minorProfile, 3)
            ] {
                let profileScore = cosine(chroma, rotatedProfile(profile, tonic: tonic))
                let triadSupport = min(
                    1,
                    (
                        chroma[tonic] * 0.45
                            + chroma[(tonic + third) % 12] * 0.30
                ...
                candidates.append(
                    KeyCandidate(
                        tonic: tonic,
                        mode: mode,
                        score: profileScore * 0.88 + triadSupport * 0.12,
                        triadSupport: triadSupport
                    )
                )
            }
        }
    ...
    private static func relativeCounterpart(
        of candidate: KeyCandidate,
        in ranked: [KeyCandidate]
    ) -> KeyCandidate? {
        let tonic = candidate.mode == "major"
            ? (candidate.tonic + 9) % 12
            : (candidate.tonic + 3) % 12
        ...
        for tonic in 0..<12 {
            for definition in modalDefinitions {
                ...
                let score = coverage * 0.58
                    ...
                    + alteredSupport * 0.14
                    + thirdSupport * 0.10
                let candidate = ModalCandidate(
                    tonic: tonic,
                    mode: definition.name,
                    score: score,
                    tonicSupport: tonicSupport,
                    alteredSupport: alteredSupport,
                    conventionalSupport: conventionalSupport
                )
    ...
    private static func temporalHalves(_ localChromas: [[Double]]) -> (first: [Double], second: [Double]) {
        ...
        let second = Array(localChromas[split...])
    ...
    private static func normalizedSum(_ chromas: [[Double]]) -> [Double] {
        ...
        for chroma in chromas {
            for pitchClass in 0..<12 {
                result[pitchClass] += chroma[pitchClass]
            }
        }
