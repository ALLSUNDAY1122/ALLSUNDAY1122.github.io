...
public enum SongSectionAnalyzer {
    ...
    public static func analyze(
        ...
        guard global.rms >= configuration.sectionSilenceRMS else {
            return [unknownSection(duration: duration)]
        }
        guard global.decidedCoverage >= configuration.minimumSectionChordCoverage else {
            return [unknownSection(duration: duration)]
        }
        ...
        if duration >= context * 2 + configuration.minimumSectionSeconds {
            ...
            while time <= duration - context + 1e-9 {
                ...
                let energyJump = min(
                    1,
                    abs(log10(localLeft.rms + 1e-6) - log10(localRight.rms + 1e-6)) * 1.5
                )
                let isEnergyCue = energyJump >= configuration.sectionEnergyJumpThreshold
                let energyBoundaryScore = isEnergyCue ? min(1, 0.55 + 0.6 * energyJump) : 0
                let strength = max(contextScore, energyBoundaryScore)

                candidates.append(
                    BoundaryCandidate(time: time, strength: strength, isEnergyCue: isEnergyCue)
                )
                time += hop
            }
        }
        ...
        for index in candidates.indices {
            ...
            let isLocalMaximum = candidate.strength >= previous && candidate.strength >= next
            guard candidate.strength >= configuration.sectionNoveltyThreshold,
                  isLocalMaximum || candidate.isEnergyCue else { continue }

            let snapped = snapToChordBoundary(
                candidate.time,
                chords: chords,
                maximumDistance: min(1, hop)
            )
            ...
            selected.append(
                BoundaryCandidate(time: snapped, strength: candidate.strength, isEnergyCue: candidate.isEnergyCue)
            )
        }
        ...
        for candidate in selected {
            ...
            let requiredSeparation = (candidate.isEnergyCue || last.isEnergyCue)
                ? min(configuration.minimumSectionSeconds, configuration.minimumEdgeSectionSeconds)
                : configuration.minimumSectionSeconds
            if candidate.time - last.time < requiredSeparation {
                ...
                if candidatePriority > lastPriority {
        ...
        var boundaries = [0.0] + filtered.map(\.time) + [duration]
        boundaries = deduplicated(boundaries)

        var segments: [SegmentWork] = []
        for index in 0..<(max(0, boundaries.count - 1)) {
            guard boundaries[index + 1] > boundaries[index] else { continue }
            segments.append(
                SegmentWork(
        ...
        for index in segments.indices {
            ...
            for candidateIndex in prototypes.indices {
                let score = descriptorSimilarity(segments[index].descriptor, prototypes[candidateIndex])
            ...
            if let bestIndex, bestSimilarity >= configuration.sectionClusterSimilarity {
                segments[index].cluster = bestIndex
                counts[bestIndex] += 1
                prototypes[bestIndex] = blend(
                    prototypes[bestIndex],
                    segments[index].descriptor,
                    previousCount: counts[bestIndex] - 1
                )
            } else {
        ...
        for index in segments.indices {
            ...
            let clusterSimilarity = descriptorSimilarity(
                segments[index].descriptor,
                prototypes[segments[index].cluster]
            )
    ...
    private static func assignFunctionalLabels(
        ...
        let energies = segments.map { $0.descriptor.rms }.sorted()
        let medianEnergy = energies[energies.count / 2]
        ...
        if segments[lastIndex].end - segments[lastIndex].start <= 12,
           segments[lastIndex].descriptor.rms < medianEnergy * 0.75,
           segments[lastIndex].confidence >= minimumConfidence {
            segments[lastIndex].functionalLabel = "outro"
        }

        let repeatedClusters = clusterCounts.indices.filter { clusterCounts[$0] >= 2 }
        // Verse/chorus semantics are intentionally withheld unless two distinct repeated
        // structural families exist. One repeated family alone is too ambiguous.
        guard repeatedClusters.count >= 2 else { return }

        let averageEnergy: [Int: Double] = Dictionary(uniqueKeysWithValues: repeatedClusters.map { cluster in
            let values = segments.filter { $0.cluster == cluster }.map { $0.descriptor.rms }
            return (cluster, values.reduce(0, +) / Double(values.count))
        })
        let ordered = repeatedClusters.sorted {
            averageEnergy[$0, default: 0] > averageEnergy[$1, default: 0]
        }

        let chorusCluster = ordered[0]
        let verseCluster = ordered[1]
        for index in segments.indices where segments[index].confidence >= minimumConfidence {
            guard segments[index].functionalLabel == nil else { continue }
            if segments[index].cluster == chorusCluster {
                segments[index].functionalLabel = "chorus"
            } else if segments[index].cluster == verseCluster {
                segments[index].functionalLabel = "verse"
            }
        }
    ...
    private static func descriptor(
        ...
        if upperSample > lowerSample {
            let maximumPoints = 8_000
            let step = max(1, (upperSample - lowerSample) / maximumPoints)
    ...
    private static func chordIndex(_ label: String) -> Int {
        ...
        let base = pitchClass[root] ?? 0
        return label.contains(":min") ? 12 + base : base
    }
    ...
    private static func descriptorDistance(_ lhs: Descriptor, _ rhs: Descriptor) -> Double {
        ...
        let energyDistance = min(
            1,
            abs(log10(lhs.rms + 1e-6) - log10(rhs.rms + 1e-6)) / 3
        )
    ...
    private static func descriptorSimilarity(_ lhs: Descriptor, _ rhs: Descriptor) -> Double {
        ...
        let energySimilarity = 1 - min(
            1,
            abs(log10(lhs.rms + 1e-6) - log10(rhs.rms + 1e-6)) / 2
        )
        let zeroCrossingSimilarity = 1 - min(
            1,
            abs(lhs.zeroCrossingRate - rhs.zeroCrossingRate) * 6
        )
    ...
    private static func snapToChordBoundary(
        _ time: Double,
        chords: [ChordEvent],
