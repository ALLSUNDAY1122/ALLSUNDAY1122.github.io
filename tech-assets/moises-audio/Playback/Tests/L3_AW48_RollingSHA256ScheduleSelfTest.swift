...
private struct Lane3AW48BitPatternSource: Lane3PCMChunkReadable, Sendable {
    let channels = 2
...
@main
struct L3AW48RollingSHA256ScheduleSelfTestMain {
    static func main() throws {
        let source = Lane3AW48BitPatternSource()
        let expected = "a44c84af8d97712aa378026410127ed758b73d0024b0aa8d053f3156291745cb"

        for chunkFrames in [1, 2, 3, 4, 7, 8] {
            let digest = try Lane3LongTrackPCMIdentityHasher.digestWithChunkVisitor(
                source,
                chunkFrames: chunkFrames
            ) { _, _, _ in }
            precondition(digest == expected, "AW48 changed canonical digest at chunkFrames=\(chunkFrames)")
        }
        ...
        print("L3-AW48 rolling SHA schedule self-test PASS digest=\(expected.prefix(16)) chunkVariants=6")
    }
}
