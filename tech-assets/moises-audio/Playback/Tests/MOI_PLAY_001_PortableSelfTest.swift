...
struct MOIPLAY001PortableSelfTest {
    ...
    static func testEffectiveGains() throws {
        ...
        precondition(gains[a] == 0.7)
        precondition(gains[b] == 0.5)
        precondition(gains[c] == 0)

        mixes[1].soloed = true
        gains = PlaybackTimelinePlanner.effectiveGains(for: mixes)
        precondition(gains[a] == 0)
        precondition(gains[b] == 0.5)
        precondition(gains[c] == 0)
    ...
    static func testControllerTransitionAndMixer() async throws {
        ...
        let controller = MultiTrackPlaybackController(
            backend: backend
        )
        let project = ProjectID()
        ...
        try await controller.prepareSource(
            projectID: project,
            asset: source
        )
        ...
        var snapshot = try await controller.snapshot(
            projectID: project
        )
        ...
        precondition(gains[vocals.id] == 0.42)
        precondition(gains[drums.id] == 0)
        ...
        precondition(gains[vocals.id] == 0)
        precondition(gains[drums.id] == 0)
        ...
        snapshot = try await controller.snapshot(
            projectID: project
        )
        precondition(
            snapshot.loop == PlaybackLoopRange(
                startSeconds: 55,
                endSeconds: 65
            )
        )
