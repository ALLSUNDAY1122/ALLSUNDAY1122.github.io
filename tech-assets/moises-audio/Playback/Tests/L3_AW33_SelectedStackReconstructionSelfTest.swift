...
private func requireReconstruction(
    _ error: Error
...
struct L3AW33SelectedStackReconstructionSelfTest {
    static func main() async throws {
        ...
        // Commit restart failure latches one-way recovery. No later operation may touch the old facade.
        ...
        do {
            ...
            // Expected first failure. The slot records the facade's recovery ticket in catch cleanup.
        }
        ...
        // A stale ticket cannot swap the selected facade.
        ...
        let receipt = try await slot.installReconstructedFacade(
            replacementFacade,
            expectedTicket: commitTicket
        )
        ...
        precondition((await replacementBackend.snapshot()).seeks == 1)

        // The old facade itself remains permanently latched; reconstruction does not reset it.
        do {
        ...
        // A non-tempo Playback poison is discovered after the operation and also requires rebuild.
        ...
        precondition((await seekPoisonSlot.snapshot()).pendingRecoveryTicket?.reason == .playbackBoundaryBackendPoisoned)

        // Cancel restart failure is also terminal. Force the DSP transaction to fail so facade takes cancel.
        let cancelBackend = AW33PlaybackBackend()
        let cancelDSP = AW33DSPBackend()
        let cancelFacade = try makeAW33Facade(
            project: ProjectID(),
            playbackBackend: cancelBackend,
            dspBackend: cancelDSP
        )
        ...
        precondition((await cancelSlot.snapshot()).pendingRecoveryTicket?.reason == .tempoBoundaryCancelFailure)
