...
public actor Lane3UnifiedPracticeControlAuthority {
    ...
    private func withShared<T>(_ op:() async -> T) async -> T {await enterSharedOperation();defer{leaveSharedOperation()};return await op()}
    private func withSharedThrowing<T>(_ op:() async throws -> T) async throws -> T {await enterSharedOperation();defer{leaveSharedOperation()};return try await op()}
