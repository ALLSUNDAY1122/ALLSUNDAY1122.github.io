...
final class IOCompatibilityDecodeBoundaryTests: XCTestCase {
    ...
    func testNonWAVOutputFailsClosedAndIsRemoved() async throws {
        try await withRoot { root, fileStore in
            ...
            XCTAssertEqual(try FileManager.default.contentsOfDirectory(at: fileStore.stagingURL, includingPropertiesForKeys: nil).filter { $0.pathExtension == "wav" }.count, 0)
        }
    }
    ...
    func testCompressedWAVFormatTagIsRejected() throws {
        try withTemporaryFile(data: makeWAV(formatTag: 6)) { url in
    ...
    private struct StubDecoder: IOCompatibilityAudioDecoding {
        ...
        func decodeToCanonicalWAV(sourceURL: URL, destinationURL: URL) async throws {
            switch mode {
            case .validPCM:
                try makeWAV(formatTag: 1).write(to: destinationURL)
            case .nonWAV:
    ...
    private func withRoot(
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("AW33-" + UUID().uuidString, isDirectory: true)
    ...
    private func withTemporaryFile(data: Data, _ body: (URL) throws -> Void) throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("AW33-WAV-" + UUID().uuidString + ".wav")
        defer { try? FileManager.default.removeItem(at: url) }
        try data.write(to: url)
        try body(url)
    }

    private static func makeWAV(formatTag: UInt16) -> Data {
        IOCompatibilityDecodeBoundaryTests().makeWAV(formatTag: formatTag)
    }

    private func makeWAV(formatTag: UInt16) -> Data {
