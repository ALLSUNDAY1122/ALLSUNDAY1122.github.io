...
- Defines `EXPECTED_RECONCILIATION_STORE_IDS` independently from the runtime tuple, so shrinking both execution and derived expectation cannot silently redefine the contract.
...
5. Shrinking the runtime reconciliation composite against the independent expected membership fails closed.
6. `lane1_dependency_audit.py` is upgraded to `L1-A26-v5` and semantically requires the A38 composite contract and its regression tokens.
7. The audit also parses the top-level `RECONCILIATION_STORE_IDS` AST and requires the exact three-store membership, so leaving an `a37_conflict_decision_store` string elsewhere cannot disguise removal from the actual composite.
8. Removing A38 contract semantics, regression semantics, membership, or paired semantics cannot false-green the dependency audit.

## Validation

Clean isolated interface-compatible execution:

- `test_mutation_topology.py`: 21/21 PASS.
- `test_lane1_dependency_audit_safety_surfaces.py`: 15/15 PASS.
- Combined focused run: 36/36 PASS, 0 failures, 0 errors.
- Dependency contract inventory reports `checked=28` in the complete focused fixture.
