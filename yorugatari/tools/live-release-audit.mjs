...
const targets = [
  ...
  { name: 'bedtime', url: `${base}/bedtime-horror.html`, required: ['寝る前に読む怖い話', '"numberOfItems":8', 'class="pick"', 'class="mood-card"', '"@type":"FAQPage"', 'assets/analytics.js?v=20260724-005'], forbidden: ['assets/engagement.js'] },
  ...
  try {
    ...
    const response = await fetch(`${target.url}${separator}verify=${Date.now()}-${attempt}`, {
      ...
      headers: {
        ...
        pragma: 'no-cache',
        'user-agent': 'Yorugatari-Live-Check/2.1'
      }
    });
