...
const analyticsPath = path.join(tools, 'analytics-clean-latest.json');
...
function number(value) { return Number.isFinite(value) ? new Intl.NumberFormat('ja-JP').format(value) : '—'; }
function delta(value) { if (!Number.isFinite(value)) return '—'; return value > 0 ? `+${number(value)}` : number(value); }
function percent(value) { return Number.isFinite(value) ? `${value.toFixed(1)}%` : '—'; }

const sourceLabels = { direct: '直接', search: '検索', social: 'SNS', referral: 'その他の参照', campaign: 'キャンペーン' };
...
const externalThreshold = readiness.thresholds?.acquisitionExternalLandings || 30;
...
const sources = (analytics.sources || []).map((row) => `| ${sourceLabels[row.channel] || row.channel} | ${number(row.views)} |`).join('\n');
const campaigns = (analytics.campaigns || []).map((row) => `| ${row.label} | ${number(row.views)} | ${campaignStatus(row)} |`).join('\n');
const conversions = (conversion.landings || []).map((row) => `| ${row.label} | ${number(row.pageViews)} | ${number(row.storyStarts)} | ${percent(row.startRate)} | ${row.comparisonReady ? '比較可能' : '収集中'} |`).join('\n');
const topStories = (analytics.topStories || []).filter((row) => row.views > 0).slice(0, 10).map((row, index) => `| ${index + 1} | ${String(row.title || '').replace(/｜夜語り$/, '')} | ${number(row.views)} |`).join('\n');
const reasons = (readiness.reasons || []).map((item) => `- ${item}`);
const analyticsActions = analytics.analysis?.actions || [];
const actions = [
  ...(publishedCampaigns.length === 0 && nextCampaign ? [`外部投稿はまだ0本。最初に「${nextCampaign.label}」をexternal-launch-kit.mdの文面とURLのまま投稿する。`] : []),
  ...analyticsActions,
  ...(conversion.actions || [])
].filter((item, index, values) => values.indexOf(item) === index).map((item) => `- ${item}`);
const externalDecision = publishedCampaigns.length === 0 ? '未投稿' : externalComparisonReady ? '実施判断可能' : '母数収集中';

const output = `# 夜語り アクセス・特集開始分析\n\n` +
  `生成日時：${analytics.generatedAt}  \n集計日（日本時間）：${analytics.japanDate || conversion.japanDate || '—'}  \nクリーン集計基準：${analytics.baseline?.establishedAt || '—'}\n\n` +
  ...
  `診断用累計：全ページ${number(analytics.rawReference?.totals?.pageViews)}件、作品${number(analytics.rawReference?.totals?.storyViews)}件。  \n` +
...
console.log(`YORUGATARI_COMBINED_INSIGHTS=${JSON.stringify({ output: outputPath, cleanBaseline: analytics.baseline?.establishedAt, publishedCampaigns: publishedCampaigns.length, externalSourceViews, externalComparisonReady, nextCampaignId: launchStatus.nextCampaignId, landingViews: conversion.totals?.landingViews, storyStarts: conversion.totals?.storyStarts, comparisonReady: conversion.comparisonReady })}`);
