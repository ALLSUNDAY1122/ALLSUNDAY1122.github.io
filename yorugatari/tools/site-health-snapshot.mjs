...
const REPORT_VERSION = '20260724-001';
...
const analyticsLoaded = readJson('analytics-snapshot-latest.json');
const conversionLoaded = readJson('landing-conversion-latest.json');
const analytics = analyticsLoaded.value || {};
const conversion = conversionLoaded.value || {};
const metrics = {
  analyticsGeneratedAt: analytics.generatedAt || null,
  pageViews: analytics.totals?.pageViews ?? null,
  storyViews: analytics.totals?.storyViews ?? null,
  sourceLandingViews: analytics.totals?.sourceLandingViews ?? null,
  externalSourceViews: analytics.analysis?.metrics?.externalSourceViews ?? null,
  campaignLandingViews: analytics.totals?.campaignLandingViews ?? null,
  notFoundViews: analytics.totals?.notFoundViews ?? null,
  activeStories: analytics.analysis?.metrics?.activeStories ?? null,
  observedDays: analytics.coverage?.observedDays ?? null,
  rankingReady: analytics.analysis?.dataReadiness?.rankingReady ?? false,
  campaignReady: analytics.analysis?.dataReadiness?.campaignReady ?? false,
...
if (!metrics.rankingReady) actions.push('人気順位や作品の並び順は変更せず、作品閲覧100件・7日分の観測まで収集を続ける。');
if ((metrics.notFoundViews ?? 0) > 0) actions.push('404発生パスを確認し、内部リンクまたはサイトマップを修正する。');
...
const metricLines = [
  `- 全ページ閲覧: ${metrics.pageViews ?? '不明'}`,
  `- 作品閲覧: ${metrics.storyViews ?? '不明'}`,
  `- 流入区分: ${metrics.sourceLandingViews ?? '不明'}`,
  `- 外部流入: ${metrics.externalSourceViews ?? '不明'}`,
  `- 固定キャンペーン流入: ${metrics.campaignLandingViews ?? '不明'}`,
  `- 404: ${metrics.notFoundViews ?? '不明'}`,
