...
async function run() {
  ...
  const context = await browser.newContext({
    ...
    isMobile: true,
    hasTouch: true,
    userAgent: 'Yorugatari-UI-Audit/1.1'
  });
  ...
  const topCards = await page.locator('.card').count();
  record('top renders 100 unique cards', topCards === 100, topCards);
  ...
  record(
    'search narrows to matching story',
    await searchCards.count() === 1 && (await searchCards.first().innerText()).includes('最後のエレベーター'),
    await searchCards.count()
  );
  ...
  record(
    ...
    categoryCount > 0 && categories.every((value) => value === '心霊'),
    { categoryCount, categories: [...new Set(categories)] }
  );
  ...
  record(
    'completed filter uses local storage',
    await completedCards.count() === 1 && (await completedCards.first().getAttribute('href')).includes('last-elevator'),
    await completedCards.count()
  );

  await page.getByRole('button', { name: 'お気に入り', exact: true }).click();
  const favoriteCards = page.locator('.card');
  record(
    'favorite filter uses local storage',
    await favoriteCards.count() === 1 && (await favoriteCards.first().getAttribute('href')).includes('neighbor-wifi'),
    await favoriteCards.count()
  );
  ...
  await Promise.all([
    randomPage.waitForURL(/stories\/last-elevator\.html/),
    randomPage.locator('#randomBtn').click()
  ]);
  ...
  await page.getByRole('button', { name: 'お気に入り', exact: true }).click();
  record(
    'archive favorite filter works',
    await page.locator('.archive-item').count() === 1 && (await page.locator('.archive-item').first().getAttribute('href')).includes('neighbor-wifi'),
    await page.locator('.archive-item').count()
  );
  ...
  record(
    'story pagination has previous archive and next links',
    await page.locator('.story-pagination a').count() === 3,
    await page.locator('.story-pagination a').allTextContents()
  );
  ...
  record(
    'explanation opens and updates state',
    await page.locator('#explanation').isVisible() && await explainButton.getAttribute('aria-expanded') === 'true'
  );
  ...
  record(
    'story favorite button persists state',
    favoriteState.includes('spare-key-returned') && await favoriteButton.getAttribute('aria-pressed') === 'true',
    favoriteState
  );
  ...
  record(
    'story completed button persists state',
    completedState.includes('spare-key-returned') && await completedButton.getAttribute('aria-pressed') === 'true',
    completedState
  );
  ...
  record(
    ...
    Boolean(readingState && readingState.slug === 'spare-key-returned' && readingState.progress > 0),
    readingState
  );
  ...
  record(
    'story desktop keeps two-column layout without overflow',
    desktopLayout.display === 'grid' && desktopLayout.columns.split(' ').length >= 2 && desktopLayout.scrollWidth <= desktopLayout.clientWidth + 1,
    desktopLayout
  );
...
(async () => {
  try {
    ...
    failures.push({ name: 'audit execution', detail: error.stack || String(error) });
  }
