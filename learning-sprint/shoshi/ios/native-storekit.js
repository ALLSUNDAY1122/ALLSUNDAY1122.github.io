...
(() => {
  ...
  function openExternal(url) {
    if (externalBridge) externalBridge.postMessage({url});
    else window.open(url, '_blank', 'noopener');
  }

  function trialCompleted() {
  ...
  function dailyGoal() {
    try {
      ...
      const goal = Number(state.dailyGoal || 8);
      return Number.isFinite(goal) ? goal : 8;
    } catch (_) {
      return 8;
    }
  }
  ...
  function canPurchase() {
    return store.native && !store.premium && !!store.displayPrice && !['product_unavailable', 'error', 'pending'].includes(store.status);
  }
  ...
  function ensurePaywall() {
    ...
    overlay.className = 'native-paywall-backdrop';
    overlay.hidden = true;
    overlay.innerHTML = `
      <section class="native-paywall" role="dialog" aria-modal="true" aria-labelledby="nativePaywallTitle">
        <div class="native-paywall-head"><div><p class="kicker">PREMIUM</p><h2 id="nativePaywallTitle">210問をすべて解放</h2></div><button class="native-paywall-close" type="button" aria-label="閉じる">×</button></div>
        <p class="muted">無料体験は「今日のスプリント」1回、最大8問です。買い切りで次を解放します。</p>
        <ul><li>令和5〜7年度の公式択一210問</li><li>年度 × 11科目の指定演習</li><li>午前・午後6枠の模試</li><li>苦手問題の個別復習</li></ul>
        <div class="native-price" data-native-price>価格を取得中</div>
        <button class="native-buy" data-native-purchase type="button" disabled>価格を取得しています</button>
        <button class="native-restore" data-native-restore type="button">購入を復元</button>
        <p class="native-paywall-foot">価格はApp Storeから取得します。このアプリは法務省の公式アプリではありません。</p>
        <div class="native-links"><button class="native-link" data-native-privacy type="button">プライバシーポリシー</button><button class="native-link" data-native-support type="button">サポート</button></div>
      </section>`;
  ...
  function ensureSettingsCard() {
    ...
    settingsCard.className = 'paper-card settings-card native-premium-card';
    ...
    const sourceNote = view.querySelector('.source-note');
    view.insertBefore(settingsCard, sourceNote || null);
  ...
  function applyStoreUI() {
    ...
    ensureSettingsCard();
    const price = statusText();
    document.querySelectorAll('[data-native-price]').forEach(el => { el.textContent = price; });
    document.querySelectorAll('[data-native-purchase]').forEach(btn => {
      btn.disabled = !canPurchase();
      btn.textContent = store.premium ? '解放済み' : canPurchase() ? `${store.displayPrice}で解放` : store.status === 'product_unavailable' || store.status === 'error' ? '価格を取得できません' : store.status === 'pending' ? '購入承認待ち' : '価格を取得しています';
    });
    if (settingsCard) {
      settingsCard.querySelector('.native-status').textContent = price;
      settingsCard.querySelector('[data-settings-buy]').textContent = store.premium ? 'プレミアム解放済み' : 'プレミアムを確認';
      settingsCard.querySelector('[data-settings-buy]').disabled = store.premium;
    }
  ...
  window.__nativeStoreKitUpdate = payload => {
    ...
    store.native = true;
    store.premium = !!payload.premium;
    store.displayPrice = typeof payload.displayPrice === 'string' ? payload.displayPrice : '';
    store.status = payload.status || 'known';
    applyStoreUI();
  };

  function gatedTarget(target) {
    return target.closest('.subject-card,.mock-card,.weak-item,#startDaily,[data-goal="16"]');
  }

  document.addEventListener('click', event => {
    if (!store.native || store.premium) return;
    const target = gatedTarget(event.target);
    if (!target) return;

    const premiumOnly = target.matches('.subject-card,.mock-card,.weak-item,[data-goal="16"]');
    const dailyLocked = target.id === 'startDaily' && (trialCompleted() || dailyGoal() > 8);
    if (!premiumOnly && !dailyLocked) return;
  ...
  function watchTrialCompletion() {
    const result = document.getElementById('resultView');
    if (!result || !store.native) return;
    const observer = new MutationObserver(() => {
      if (!store.premium && result.classList.contains('active')) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    ensurePaywall();
    ensureSettingsCard();
    watchTrialCompletion();
    const offline = document.getElementById('offlineStatus');
    if (offline) offline.textContent = '210問と画面をアプリ本体に同梱しています。通信なしでも学習できます。';
    applyStoreUI();
    send('refresh');
  });
})();
