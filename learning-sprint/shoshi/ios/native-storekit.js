...
(() => {
  ...
  function dailyGoal() { try { const s=JSON.parse(localStorage.getItem(APP_STATE_KEY)||'{}'); const g=Number(s.dailyGoal||8); return Number.isFinite(g)?g:8; } catch (_) { return 8; } }
  function statusText() { if(store.premium)return 'プレミアム解放済み'; if(store.displayPrice)return `買い切り ${store.displayPrice}`; if(store.status==='product_unavailable'||store.status==='error')return '価格を取得できません'; if(store.status==='pending')return '購入承認待ち'; return '価格を取得中'; }
  ...
  function injectStyles() {
    ...
    const style=document.createElement('style'); style.id='nativePremiumStyles';
    style.textContent=`
      ...
      .native-premium-card{margin-top:14px}.native-premium-card .native-status{font-weight:900;color:#2f4a6d;margin:.35rem 0 .8rem}.native-premium-card button{min-height:46px}
  ...
  function ensurePaywall() {
    ...
    overlay.querySelector('.native-paywall-close').addEventListener('click',hidePaywall); overlay.addEventListener('click',e=>{if(e.target===overlay)hidePaywall();});
    overlay.querySelector('[data-native-purchase]').addEventListener('click',()=>send('purchase')); overlay.querySelector('[data-native-restore]').addEventListener('click',()=>send('restore'));
    overlay.querySelector('[data-native-privacy]').addEventListener('click',()=>openExternal('https://allsunday1122.github.io/learning-sprint/shoshi/privacy/')); overlay.querySelector('[data-native-support]').addEventListener('click',()=>openExternal('https://allsunday1122.github.io/learning-sprint/shoshi/support/'));
    return overlay;
  }
  ...
  function ensureSettingsCard(){if(!store.native||settingsCard)return;const view=document.getElementById('settingsView');if(!view)return;settingsCard=document.createElement('article');settingsCard.className='paper-card settings-card native-premium-card';settingsCard.innerHTML=`<label>プレミアム</label><p class="native-status">価格を取得中</p><p class="muted">無料体験後、買い切りで210問・科目別・模試・苦手復習を解放します。</p><div class="stack-actions"><button class="secondary-button" data-settings-buy type="button">プレミアムを確認</button><button class="secondary-button" data-settings-restore type="button">購入を復元</button></div>`;const sourceNote=view.querySelector('.source-note');view.insertBefore(settingsCard,sourceNote||null);settingsCard.querySelector('[data-settings-buy]').addEventListener('click',showPaywall);settingsCard.querySelector('[data-settings-restore]').addEventListener('click',()=>send('restore'));}
  function applyStoreUI(){if(!store.native)return;ensureSettingsCard();const price=statusText();document.querySelectorAll('[data-native-price]').forEach(el=>{el.textContent=price;});document.querySelectorAll('[data-native-purchase]').forEach(btn=>{btn.disabled=!canPurchase();btn.textContent=store.premium?'解放済み':canPurchase()?`${store.displayPrice}で解放`:store.status==='product_unavailable'||store.status==='error'?'価格を取得できません':store.status==='pending'?'購入承認待ち':'価格を取得しています';});if(settingsCard){settingsCard.querySelector('.native-status').textContent=price;settingsCard.querySelector('[data-settings-buy]').textContent=store.premium?'プレミアム解放済み':'プレミアムを確認';settingsCard.querySelector('[data-settings-buy]').disabled=store.premium;}if(store.premium)hidePaywall();}
