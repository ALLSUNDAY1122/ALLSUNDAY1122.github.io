...
for(const q of QUESTIONS){
  ...
  if(['指定数量計算','指定数量応用'].includes(q.topic)){
    const cat=Object.keys(qty).find(k=>q.question.includes(k)); const m=q.question.match(/([\d,]+) L/);
    if(!cat||!m)fail(q.id,'cannot parse single quantity calculation');
    else {const x=Number(m[1].replace(/,/g,''))/qty[cat], y=Number(String(ans(q)).replace('倍','')); if(Math.abs(x-y)>1e-9)fail(q.id,`quantity multiple ${y}, expected ${x}`)}
  }
  if(q.topic==='倍数合算'){
    ...
    total=Math.round(total*100)/100; const y=Number(String(ans(q)).replace('倍',''));
...
if(QUESTIONS.length!==expected.total)errors.push(`total ${QUESTIONS.length} != 360`);
