...
const chapterSessionKey=chapter=>`chapter:${chapter}`;
...
function todayQuestions(){
  const weakIds=new Set(weakList().map(q=>q.id));
  const chapterOrder=shuffle(CHAPTERS);
  const buckets={};
  chapterOrder.forEach(c=>{
    const qs=QUESTIONS.filter(q=>q.chapter===c.key);
    const unseen=shuffle(qs.filter(q=>!STATE.seenIds[q.id]));
    const weak=shuffle(qs.filter(q=>STATE.seenIds[q.id]&&weakIds.has(q.id)));
    const seen=shuffle(qs.filter(q=>STATE.seenIds[q.id]&&!weakIds.has(q.id)));
    buckets[c.key]=[...unseen,...weak,...seen];
  });
  const out=[];
  while(out.length<12){
    let moved=false;
    for(const c of chapterOrder){
      const q=buckets[c.key].shift();
      if(q){out.push(q);moved=true;if(out.length===12)break}
    }
    if(!moved)break;
  }
  return out;
...
function renderHome(){
  ...
  let h=`<div class="home-top"><div class="brand-tag">登録販売者試験対策</div><div class="brand-title">登録販売者｜学びスプリント</div><p class="brand-sub">令和8年4月手引き準拠・本番配分120問</p></div><div class="hero"><p class="hero-title">今日も1問、力に変える。</p></div>`;
  ...
  h+=`<section class="section"><h2>クイック学習</h2>
    <button class="full-btn" data-action="today"><span class="emoji">⚡</span><span><span class="name">今日の12問</span><span class="meta">全5章からバランス出題・完走 ${completionCount(SESSION_TODAY)} 回</span></span></button>
  ...
  h+=`<section class="section"><h2>章別で学ぶ</h2><div class="chapter-grid">${CHAPTERS.map(c=>{const n=QUESTIONS.filter(q=>q.chapter===c.key).length;return `<button class="chapter-card active" data-chapter="${c.key}" style="--ch-accent:${c.accent};--ch-soft:${c.soft}">${ring(completionCount(chapterSessionKey(c.key)),c.accent)}<span class="chapter-info"><span class="chapter-name">${c.key}<br>${c.title}</span><span class="chapter-meta">${n}問</span></span></button>`}).join('')}</div><p class="section-note">本試験の分野別配分20・20・40・20・20問に合わせた全120問。誤答と「わからない」は自動で苦手復習へ送られます。</p></section>`;
  h+=`<section class="section"><h2>本番模試</h2><details class="mock-mode"><summary><span>本番模試モード</span><span class="mock-meta">120問・240分目安</span></summary><div class="mock-body"><p>5分野を20・20・40・20・20問で出題します。合否基準の分野別足切りは都道府県で異なるため、アプリ内では合否を断定しません。</p><button class="next-btn" data-action="mock">120問模試を開始</button></div></details></section>`;
  ...
  app.querySelector('[data-action="today"]').onclick=()=>startSession(SESSION_TODAY,todayQuestions(),'今日の12問');
  const mb=app.querySelector('[data-action="mock"]');if(mb)mb.onclick=()=>startSession(SESSION_MOCK,shuffle(QUESTIONS),'本番模試｜120問');
  app.querySelectorAll('[data-chapter]').forEach(b=>b.onclick=()=>{const c=CHAPTERS.find(x=>x.key===b.dataset.chapter);const qs=QUESTIONS.filter(q=>q.chapter===c.key);startSession(chapterSessionKey(c.key),qs,`${c.key}｜${c.title}`)});
...
function startSession(key,questions,title){if(!questions.length)return;SESSION={key,title,questions,index:0,correct:0,chapterCorrect:{},answeredCount:0,answered:false,selected:null,startedAt:Date.now()};saveProgress();go('quiz')}
function saveProgress(){if(!SESSION)return;STATE.inProgress={key:SESSION.key,title:SESSION.title,ids:SESSION.questions.map(q=>q.id),idx:SESSION.index,correct:SESSION.correct,chapterCorrect:SESSION.chapterCorrect||{},answeredCount:SESSION.answeredCount,answered:SESSION.answered,selected:SESSION.selected,startedAt:SESSION.startedAt};saveState()}
function clearProgress(){STATE.inProgress=null;saveState()}
function resumeSession(){const ip=STATE.inProgress;if(!ip)return;const qs=ip.ids.map(id=>byId[id]);if(qs.some(q=>!q)||ip.idx<0||ip.idx>=qs.length){clearProgress();go('home');alert('前回の学習データを復元できませんでした。');return}SESSION={key:ip.key,title:ip.title,questions:qs,index:ip.idx,correct:ip.correct||0,chapterCorrect:isPlainObject(ip.chapterCorrect)?ip.chapterCorrect:{},answeredCount:ip.answeredCount||0,answered:!!ip.answered,selected:Number.isInteger(ip.selected)?ip.selected:null,startedAt:ip.startedAt||Date.now()};go('quiz')}
...
function answerQuestion(choice){if(SESSION.answered)return;const q=SESSION.questions[SESSION.index],correct=choice===q.answer;SESSION.selected=choice;SESSION.answered=true;SESSION.answeredCount++;STATE.seenIds[q.id]=true;STATE.stats.totalAnswered++;if(correct){SESSION.correct++;SESSION.chapterCorrect=SESSION.chapterCorrect||{};SESSION.chapterCorrect[q.chapter]=(SESSION.chapterCorrect[q.chapter]||0)+1;STATE.stats.totalCorrect++;const w=STATE.weak[q.id];if(w){const streak=(w.streak||0)+1;if(streak>=3)delete STATE.weak[q.id];else STATE.weak[q.id]={streak}}}else STATE.weak[q.id]={streak:0};saveProgress();renderQuiz()}
...
function renderResult(){const total=SESSION.questions.length,rate=Math.round(SESSION.correct/total*100);let h=`<div class="header"><h1>結果</h1></div><div class="result-card"><div class="result-big">${SESSION.correct} / ${total}</div><div class="result-label">${esc(SESSION.title)} 完走！</div><div class="result-detail"><div><div class="n">${rate}%</div><div class="l">正答率</div></div><div><div class="n">${weakList().length}</div><div class="l">現在の弱点数</div></div><div><div class="n">${completionCount(SESSION.key)}</div><div class="l">完走回数</div></div></div></div>`;
  if(SESSION.key===SESSION_MOCK){h+=`<section class="section"><h2>分野別得点</h2><div class="mock-score-grid">${CHAPTERS.map(c=>{const totalC=QUESTIONS.filter(q=>q.chapter===c.key).length;const got=(SESSION.chapterCorrect&&SESSION.chapterCorrect[c.key])||0;return `<div class="mock-score"><span>${c.key}</span><strong>${got}/${totalC}</strong></div>`}).join('')}</div><p class="section-note">総得点70％程度に加え、分野別基準は都道府県知事が定めます。この画面では合否判定を断定しません。</p></section>`}
...
function renderSettings(){let h=`<div class="header"><h1>設定</h1></div><section class="section"><div class="settings-row"><div class="label">文字サイズ</div><div class="seg">${[['normal','標準'],['large','大'],['xlarge','特大']].map(([k,l])=>`<button data-fs="${k}" class="${STATE.settings.fontSize===k?'active':''}">${l}</button>`).join('')}</div></div><div class="settings-row"><div class="label">教材基準</div><div class="settings-note">厚生労働省「試験問題の作成に関する手引き」令和8年4月一部改訂版を基準に管理しています。現在版は全5章120問を本試験の分野配分20・20・40・20・20問で収録しています。</div></div><div class="settings-row"><div class="label">データについて</div><div class="settings-note">学習記録・弱点情報はこの端末内にのみ保存します。ログイン、広告、氏名・メールアドレス等の個人情報保存はありません。</div></div><div class="settings-row"><div class="label">学習記録のリセット</div><button class="danger-btn" id="resetBtn">すべての学習記録をリセット</button></div></section>${tabbar('settings')}`;app.innerHTML=h;app.querySelectorAll('[data-fs]').forEach(b=>b.onclick=()=>{STATE.settings.fontSize=b.dataset.fs;saveState();applyFont();renderSettings()});document.getElementById('resetBtn').onclick=()=>{if(confirm('本当にすべての学習記録をリセットしますか？')){try{localStorage.removeItem(LS_KEY);localStorage.removeItem(LEGACY_KEY)}catch(e){}STATE=freshState();applyFont();go('home')}};bindTabs()}
