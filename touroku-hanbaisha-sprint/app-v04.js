...
let h=`<div class="home-top"><div class="brand-tag">登録販売者試験対策</div><div class="brand-title">登録販売者｜学びスプリント</div><p class="brand-sub">令和8年4月手引き準拠・全5章60問</p></div><div class="hero"><p class="hero-title">今日も1問、力に変える。</p></div>`;
...
h+=`<section class="section"><h2>章別で学ぶ</h2><div class="chapter-grid">${CHAPTERS.map(c=>`<button class="chapter-card active" data-chapter="${c.key}" style="--ch-accent:${c.accent};--ch-soft:${c.soft}">${ring(completionCount(chapterSessionKey(c.key)),c.accent)}<span class="chapter-info"><span class="chapter-name">${c.key}<br>${c.title}</span><span class="chapter-meta">12問</span></span></button>`).join('')}</div><p class="section-note">各章12問・全60問。誤答と「わからない」は自動で苦手復習へ送られます。</p></section>`;
...
function renderSettings(){let h=`<div class="header"><h1>設定</h1></div><section class="section"><div class="settings-row"><div class="label">文字サイズ</div><div class="seg">${[['normal','標準'],['large','大'],['xlarge','特大']].map(([k,l])=>`<button data-fs="${k}" class="${STATE.settings.fontSize===k?'active':''}">${l}</button>`).join('')}</div></div><div class="settings-row"><div class="label">教材基準</div><div class="settings-note">厚生労働省「試験問題の作成に関する手引き」令和8年4月一部改訂版を基準に管理しています。現在版は監査済み全5章60問を収録しています。</div></div><div class="settings-row"><div class="label">データについて</div><div class="settings-note">学習記録・弱点情報はこの端末内にのみ保存します。ログイン、広告、氏名・メールアドレス等の個人情報保存はありません。</div></div><div class="settings-row"><div class="label">学習記録のリセット</div><button class="danger-btn" id="resetBtn">すべての学習記録をリセット</button></div></section>${tabbar('settings')}`;app.innerHTML=h;app.querySelectorAll('[data-fs]').forEach(b=>b.onclick=()=>{STATE.settings.fontSize=b.dataset.fs;saveState();applyFont();renderSettings()});document.getElementById('resetBtn').onclick=()=>{if(confirm('本当にすべての学習記録をリセットしますか？')){try{localStorage.removeItem(LS_KEY);localStorage.removeItem(LEGACY_KEY)}catch(e){}STATE=freshState();applyFont();go('home')}};bindTabs()}
applyFont();render();
})();
