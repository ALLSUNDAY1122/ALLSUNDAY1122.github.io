...
# Keep deterministic initial release numbering.
perl -0pi -e 's/MARKETING_VERSION = [^;]+;/MARKETING_VERSION = 1.0.0;/g' "$PROJECT"
perl -0pi -e 's/CURRENT_PROJECT_VERSION = [^;]+;/CURRENT_PROJECT_VERSION = 1;/g' "$PROJECT"
...
echo "Configured iOS target: privacy manifest, export compliance, iPhone-only, version 1.0.0 (1)."
