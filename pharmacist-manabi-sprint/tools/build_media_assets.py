...
def raw_questions():
    ...
    for exam in (111,110,109):
        ...
        out.update({q["id"]:q for q in d["questions"]})
    return out
...
def bbox_pages(pdf: Path):
    ...
    for pe in [x for x in root.iter() if lname(x.tag)=='page']:
        ...
        for le in [x for x in pe.iter() if lname(x.tag)=='line']:
            ...
            text=''.join((w.text or '') for w in words).replace('\u3000',' ')
            y=float(le.attrib.get('yMin',words[0].attrib.get('yMin','0')))
            lines.append((text,y))
...
def rendered_page(pdf:Path,key:str,pageno:int):
    prefix=PAGE_CACHE/f'{key}-p{pageno+1:03d}'
    jpg=Path(str(prefix)+'.jpg')
    if not jpg.exists():
        subprocess.run(['pdftoppm','-f',str(pageno+1),'-l',str(pageno+1),'-r',str(DPI),'-jpeg','-singlefile',str(pdf),str(prefix)],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    return Image.open(jpg).convert('RGB')


def crop_segment(pdf,key,pages,pageno,top,bottom,out):
    img=rendered_page(pdf,key,pageno)
    meta=pages[pageno]
    sx=img.width/meta['width']; sy=img.height/meta['height']
    x0=max(0,int(16*sx)); x1=min(img.width,int((meta['width']-16)*sx))
    y0=max(0,int(top*sy)); y1=min(img.height,int(bottom*sy))
...
def main():
    mids=set(media_ids()); qmap=raw_questions(); manifest={}; third=[]; failures=[]; exported=0
    grouped={}
    for qid in sorted(mids):
        q=qmap[qid]; exam=int(q['sourceExam']); n=int(q['questionNo']); ex,part=source_for(exam,n)
    ...
    for (exam,part_id,url,start,end),ids in grouped.items():
        ...
        try:
            pages=bbox_pages(pdf); markers=markers_from_bbox(pages,start,end)
        except Exception as e:
            failures.append({'part':f'{exam}-{part_id}','reason':f'bbox failure {e!r}'}); continue
        if len(markers)<end-start+1:
            failures.append({'part':f'{exam}-{part_id}','markerMissing':[n for n in range(start,end+1) if n not in markers]})
        for qid in ids:
            q=qmap[qid]; n=int(q['questionNo']); text=(q.get('question','')+' '+' '.join(q.get('choices',[])))
            reason=KNOWN_THIRD_PARTY.get(qid)
            if not reason and THIRD_RE.search(text): reason='source-marker-detected-in-extracted-text'
            if reason:
                third.append({'id':qid,'exam':exam,'questionNo':n,'reason':reason,'sourceUrl':url}); continue
            if n not in markers:
            ...
            try:
                for pno in range(sp,ep+1):
                    top=max(8,sy-7) if pno==sp else 16
                    bottom=(ey-7 if pno==ep else pages[pno]['height']-16)
                    out=ASSET_ROOT/str(exam)/f'q{n:03d}-{len(assets)+1}.webp'
                    crop_segment(pdf,f'{exam}-{part_id}',pages,pno,top,bottom,out)
                    assets.append(str(out.relative_to(ROOT)).replace('\\','/'))
            except Exception as e:
                failures.append({'id':qid,'reason':repr(e)}); continue
            manifest[qid]={
                'assets':assets,'sourceUrl':url,
                'attribution':f"出典：厚生労働省『第{exam}回薬剤師国家試験問題及び解答』",
                'modified':True,'license':'PDL1.0','releaseStatus':'asset_ready_pending_final_visual_review'
            }; exported+=1
