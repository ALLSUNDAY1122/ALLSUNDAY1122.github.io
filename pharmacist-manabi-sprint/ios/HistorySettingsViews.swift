        ...
        SprintCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if learning.state.weak.isEmpty {
                    Text("苦手問題はまだありません。")
                        .font(.system(size: 13)).foregroundStyle(Color.sprintInk3).padding(.vertical, 8)
                } else {
                        ...
                        if let q = learning.questionMap[id], let w = learning.state.weak[id] {
                            HStack {
                                Text("\(q.field)・問\(q.questionNo)")
                                    .font(.system(size: 13, weight: .semibold)).foregroundStyle(Color.sprintInk)
                                Spacer()
                                Text("\(w.streak)/3")
                                    .font(.system(size: 13, weight: .bold)).foregroundStyle(Color.sprintShu)
                            }
                            .padding(.vertical, 7)
                            Divider().overlay(Color.sprintLine)
                        }
...
struct SettingsView: View {
            ...
            VStack(spacing: 14) {
                ...
                SprintCard {
                    VStack(spacing: 0) {
                        settingBlock("文字サイズ") {
                            HStack(spacing: 6) {
                                ForEach([16,18,20], id: \.self) { size in
                                    segmentButton(label: size == 16 ? "標準" : size == 18 ? "大" : "特大", active: learning.state.fontSize == size) { learning.updateFontSize(size) }
                                }
                            }
                        }
                        divider
                        ...
                        premiumBlock
                        divider
                        infoBlock(title: "この教材について", text: "第111・110・109回の監査済み問題バンク1,035問を端末内に同梱。ログイン・広告・独自クラウド同期はありません。")
                        divider
    ...
    private func segmentButton(label: String, active: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(label).font(.system(size: 12, weight: .bold)).foregroundStyle(active ? Color.sprintAi : Color.sprintInk3)
    ...
    private var examDateBlock: some View {
        VStack(alignment: .leading, spacing: 10) {
            Toggle("試験日を設定", isOn: $useExamDate)
                .font(.system(size: 13, weight: .semibold)).tint(Color.sprintAi)
    ...
    private var premiumBlock: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("プレミアム").font(.system(size: 13, weight: .bold)).foregroundStyle(Color.sprintInk)
                Spacer()
                if storeKit.isPremium { Text("利用中").font(.system(size: 11, weight: .bold)).foregroundStyle(Color.sprintMidori) }
            }
            Text(storeKit.isPremium ? "3回分1,031採点対象問題が解放されています。" : "無料版は第111回必須90問。プレミアムで第111・110・109回の採点対象1,031問を解放します。")
                .font(.system(size: 12)).foregroundStyle(Color.sprintInk2)
            if !storeKit.isPremium {
                Button("プランを見る") { learning.paywallPresented = true }
                    .font(.system(size: 13, weight: .bold)).foregroundStyle(Color.sprintAi)
            }
            HStack {
