...
final class QuestionModelTests: XCTestCase {
    private func question(
        ...
        Question(
            id: id, exam: exam, questionNo: 1, section: section, field: "物理・化学・生物",
            question: "テスト問題", choices: ["A", "B", "C", "D", "E"], answer: answer,
            acceptedAnswers: accepted, scoringStatus: accepted.isEmpty ? "normal" : "multiple_accepted",
            memoryPoint: "要点", explanation: "解説", sharedStem: "", displayMode: "textChoices",
            mediaAssets: [], numberedChoiceCount: 0, attribution: "出典", modificationDisclosure: "加工して作成",
            canonicalId: id
        )
    }
    ...
    func testNoIAPReleaseHasAllScoredQuestionsAvailable() {
        ...
        let all = [111, 110, 109].flatMap { exam in
            ["必須", "理論", "実践"].flatMap { section in
                store.sectionQuestions(exam: exam, section: section, premium: true)
            }
        }
        XCTAssertEqual(all.count, 1031)
        XCTAssertEqual(Set(all.map(\.id)).count, 1031)
    }
    ...
    func testDailySprintMatchesOfficialSectionRatio() {
        ...
        for expected in cases {
            ...
            let sections = (store.activeSession?.ids ?? []).compactMap { store.questionMap[$0]?.section }
    ...
    @MainActor
    func testFieldBatchesCoverEveryQuestionWithoutEightQuestionLimit() {
        let store = LearningStore()
        store.resetLearningData()
        store.updateShuffleQuestions(false)
        store.updateGoal(8)
        ...
        XCTAssertNotEqual(store.activeSession?.ids.count, store.state.goal)
    ...
    func testWrongOrUnknownAnswerStillAdvancesDailyHeatmapAndAchievement() {
        ...
        XCTAssertEqual(store.todayRecord.answered, 1)
        XCTAssertGreaterThan(store.learningProgress, before)
