...
@main struct StorageAuditRunner {
  static func main() throws {
    ...
    let report = ProcessingStorageLifecycleAuditor().audit(mediaImportSource: media, productFlowStoreSource: store, productionRuntimeSource: runtime, appResourceTexts: resources)
    if !report.pass {
