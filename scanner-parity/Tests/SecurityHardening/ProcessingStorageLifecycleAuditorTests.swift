...
struct ProcessingStorageLifecycleAuditorTests {
    static func main() throws {
        ...
        test("managed recovery storage passes with explicit lifecycle") {
        ...
        test("generic removeItem is not enough to prove workspace purge") {
            let r = auditor.audit(mediaImportSource: managedImport, productFlowStoreSource: "applicationSupportDirectory removeItem(at: workspace)", productionRuntimeSource: runtime, appResourceTexts: [])
            try require(r.issues.contains(.processingWorkspaceNotPurged))
        }
