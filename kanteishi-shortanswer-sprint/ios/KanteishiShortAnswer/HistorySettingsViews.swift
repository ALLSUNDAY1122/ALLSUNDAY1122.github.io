...
struct HistoryView: View {
    ...
    var body: some View {
        let accessibleWeak = store.accessibleWeakQuestions(isPremium: purchases.isPremium)
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    HStack(spacing: 16) {
                        ...
                        VStack(spacing: 10) {
                            ...
                            historyStat("\(accessibleWeak.count)", "苦手")
                        }
                    }
                    .padding(16)
                    .appCard()
                    ...
                    LazyVGrid(columns: heatColumns, spacing: 4) {
                        ForEach(Array(store.heatmap().enumerated()), id: \.offset) { _, item in
                            RoundedRectangle(cornerRadius: 4, style: .continuous)
                                .fill(heatColor(item.1))
                                .aspectRatio(1, contentMode: .fit)
                                .accessibilityLabel("\(item.0.formatted(date: .abbreviated, time: .omitted))、\(item.1)問")
                        }
                    }
                    .padding(14)
                    .appCard()
                    ...
                    if accessibleWeak.isEmpty {
                        Text("現在利用できる苦手問題はありません。")
                            .appSans(13)
                        ...
                        ForEach(accessibleWeak) { q in
                            Button {
                                if !store.retryQuestions([q.id], title: q.topic, isPremium: purchases.isPremium) {
                                ...
                                HStack {
                                    ...
                                    Spacer()
                                    Text("連続 \(store.state.weak[q.id]?.streak ?? 0)/3")
                                        .appSans(11, weight: .bold)
    ...
    private func historyStat(_ value: String, _ label: String) -> some View {
        HStack {
            Text(value).appSerif(19, weight: .bold).foregroundStyle(AppTheme.ink)
            Spacer()
            Text(label).appSans(11, weight: .bold).foregroundStyle(AppTheme.ink3)
        }
    }
...
struct SettingsView: View {
    ...
    var body: some View {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    settingsSection("文字サイズ") {
                        Picker(
                            ...
                            ForEach(FontSizePreference.allCases) { Text($0.title).tag($0) }
                        }
                        .pickerStyle(.segmented)
                    }
                    ...
                    settingsSection("試験日") {
                        ...
                        .tint(AppTheme.ai)
                        if let date = store.settings.examDate {
                            DatePicker(
                                "試験日",
                                selection: Binding(get: { date }, set: { store.setExamDate($0) }),
                    ...
                    settingsSection("このアプリ") {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            infoRow("監査基準日", store.repository.payload.sourceCheckedAt)
                            infoRow("収録問題", "240問（令和8・7・6年）")
                            infoRow("構成", "各年度 行政法規40＋鑑定理論40")
                            infoRow(
                                "課金",
                                purchases.isConfigured
                                    ? "StoreKit 2・月額プラン"
                                    : "Product ID 未設定（Release Gate）"
                            )
                            infoRow("Bundle ID", "jp.allsunday1122.kanteishishortanswer")
                        }
                    }

                    Text("本アプリは国土交通省・土地鑑定委員会の公式アプリではありません。収録問題は国土交通省が公表した令和8・7・6年の不動産鑑定士試験短答式試験問題を、公共データ利用規約（PDL1.0）に基づきアプリ表示用に構造化して収録しています。")
                        .appSans(11)
                        .foregroundStyle(AppTheme.ink3)
                        .padding(.bottom, 18)
                }
        ...
        .fileExporter(
            ...
            contentType: .json,
            defaultFilename: "kanteishi-shortanswer-backup"
        ) { result in
    ...
    private func settingsSection<Content: View>(
        ...
        VStack(alignment: .leading, spacing: 10) {
            Text(title).appSerif(18, weight: .bold).foregroundStyle(AppTheme.ink)
            content()
        }
        .padding(16)
        .appCard()
    ...
    private func settingsAction(_ image: String, _ title: String) -> some View {
        HStack {
            Image(systemName: image).foregroundStyle(AppTheme.ai).frame(width: 24)
            Text(title).appSans(14, weight: .bold).foregroundStyle(AppTheme.ink)
            Spacer()
