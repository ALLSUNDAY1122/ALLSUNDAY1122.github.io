            ...
            ScrollView{
                VStack(alignment:.leading,spacing:18){
                    ...
                    settingsSection("このアプリ") {
                        VStack(alignment:.leading,spacing:10){ infoRow("コンテンツ",store.repository.payload.contentVersion);infoRow("監査記録日時",store.repository.payload.sourceCheckedAt);infoRow("試作問題","12問（公開不可）");infoRow("製品版固定枠","240問");infoRow("課金",purchases.isConfigured ? "StoreKit 2" : "Product ID 未設定（Release Gate）");infoRow("Bundle ID","未設定（推測禁止）") }
                    }
