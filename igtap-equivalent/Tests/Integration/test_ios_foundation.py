...
class IOSFoundationTests(unittest.TestCase):
    ...
    def test_ios_project_has_mobile_texture_import_and_original_icon(self):
        project = self.read("project.godot")
        icon = ROOT / "Platform/iOS/AppIcon.svg"
        self.assertIn('textures/vram_compression/import_etc2_astc=true', project)
        self.assertIn('config/icon="res://Platform/iOS/AppIcon.svg"', project)
