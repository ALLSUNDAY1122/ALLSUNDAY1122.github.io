...
def main() -> int:
    ...
    media_ids, scoring_ids, expert_ids, release_ids = set(), set(), set(), set()

    for qid, q in questions.items():
        reasons = q.get("specialistQuarantineReasons") or []
        kinds = {x.get("kind") for x in reasons}
        is_media = "mediaDependent" in kinds or bool(q.get("requiresMedia"))
        ...
        if q.get("answerType") in {"singleChoice", "multiChoice"} and not choices and not is_media:
            errors.append(f"choices_missing_non_media:{qid}")
    ...
    checks = {
        ...
        "source_answer_token_mismatch": token_mismatches,
    ...
    expected_counts = {
        "media": (len(media_ids), 8),
        "scoring": (len(scoring_ids), 6),
        "expert": (len(expert_ids), 2),
        "quarantine": (len(quarantine), 14),
        "release": (len(release_ids), 136),
    }
    for label, (actual, expected) in expected_counts.items():
        if actual != expected:
            errors.append(f"{label}_count:{actual}!={expected}")

    payloads = {qid: payload(q) for qid, q in questions.items()}
    groups = defaultdict(list)
    for qid, text in payloads.items():
        groups[text].append(qid)
    exact_duplicates = [sorted(ids) for text, ids in groups.items() if text and len(ids) > 1]
    if exact_duplicates:
    ...
    for i, a in enumerate(ids):
        ...
        for b in ids[i + 1:]:
            ...
            if len(pb) < 16:
                continue
            if min(len(pa), len(pb)) / max(len(pa), len(pb)) < HIGH_SIM_THRESHOLD:
                continue
    ...
    report = {
        ...
        "policy": "canonical JSON is final specialist source; batch-level PASS is not reused as final PASS; media-only choices are not fabricated",
        "counts": {
            "requiredTotal": len(questions),
            "byExam": {str(exam): exam_counts[exam] for exam in EXAMS},
            "answerMismatches": len(set(answer_mismatches)),
            "sourceAnswerTokenMismatches": len(set(token_mismatches)),
