...
for set_id in ('set1','set2','set3'):
    ...
    ids=set(); cats={k:0 for k in expected}; media=0; empty_stem=0; missing_answers=0; exclusions=0; multiple_accepted=0; numeric=0
    seen_exceptions=set()
    for q in qs:
        qid=q.get('id')
...
report={
    ...
    'note':'raw取込監査。採点除外・複数正解は公式例外として保持する。重複は最終720問化の前に出題実績と水増しを区別して判断する。'
}
