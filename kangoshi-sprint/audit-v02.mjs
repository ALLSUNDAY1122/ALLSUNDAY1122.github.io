...
for(const name of ['questions.js','questions-numeric.js','questions-audit-v1.js','question-taxonomy-v1.js'])vm.runInContext(fs.readFileSync(new URL(`./${name}`,import.meta.url),'utf8'),context);
...
console.log(`OK: ${q.length} audited questions / 24-24-12 / types ${JSON.stringify(types)} / no trivial distractors / official 11-subject taxonomy only`);
