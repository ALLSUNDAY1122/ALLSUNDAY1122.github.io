...
export default function CardsScreen() {
  ...
  const confirmClear = () => {
    Alert.alert('すべて削除', 'カードと学習履歴をすべて削除しますか？', [
      { text: 'キャンセル', style: 'cancel' },
      { text: '次へ', style: 'destructive', onPress: () => Alert.alert('最終確認', 'この操作は取り消せません。', [
        { text: 'キャンセル', style: 'cancel' },
  ...
  return (
    <Page>
      ...
      <Section title="カード一覧" right={cards.length ? <AppButton label="全削除" variant="danger" onPress={confirmClear} /> : null}>
        <ChoiceRow value={filter} onChange={setFilter} options={[
          { value: 'all', label: 'すべて' },
          { value: 'weak', label: '苦手' },
          ...
          <View key={card.id} style={styles.cardRow}>
            {editingId === card.id ? <>
              ...
              <View style={commonStyles.row}>
                ...
                <AppButton label="キャンセル" variant="secondary" onPress={() => setEditingId(null)} />
              </View>
            </> : <>
