...
def main() -> None:
    ...
    require("NetworkSpecialist/Resources" in project, "native Resources directory not bundled")
    require("WebApp" not in project, "WebApp resource must not be bundled in native target")

    swift_files = list((root / "ios" / "NetworkSpecialist").glob("*.swift"))
    ...
    require("submit_to_testflight: false" in codemagic, "Beta review submission must remain disabled for internal-only TestFlight")
    ...
    print("PASS native release static gate")
