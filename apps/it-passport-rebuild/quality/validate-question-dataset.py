...
def main() -> None:
    ...
    for i, q in enumerate(questions, 1):
        ...
        require(qid not in seen, f"duplicate id: {qid}")
        seen.add(qid)
        ...
        require(isinstance(q["question"], str) and q["question"].strip(), f"{qid}: question empty")
        ...
        rnd = str(q["round"]).strip()
        require(rnd, f"{qid}: round empty")
        round_counts[rnd] += 1
    ...
    if args.release:
        ...
        print(f"PASS: IT Passport development dataset schema valid; current={len(questions)} target={target} live_syllabus={live_syllabus}")
